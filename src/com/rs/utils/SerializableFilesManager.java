...
public class SerializableFilesManager {
	...
	public static boolean createBackup(String username) {
		try {
			Utils.copyFile(new File(PATH + username.toLowerCase() + ".p"), new File(
					BACKUP_PATH + username.toLowerCase() + ".p"));
			return true;
		} catch (Throwable e) {
			Logger.handle(e);
			return false;
	...
	public synchronized static void savePlayer(Player player) {
		try {
			...
			storeSerializableClass(player, new File(PATH + player.getUsername().toLowerCase()
					+ ".p"));
