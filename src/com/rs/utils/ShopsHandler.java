...
public class ShopsHandler {
	...
	private static void loadPackedShops() {
		try {
			...
			Item items[];
			for (ByteBuffer buffer = channel.map(
					java.nio.channels.FileChannel.MapMode.READ_ONLY, 0L,
					channel.size()); buffer.hasRemaining(); addShop(key,
					new Shop(name, money, items, generalStore))) {
				key = buffer.getInt();
	...
	public static void restoreShops() {
		Shop shop;
		for (Iterator<Shop> iterator = handledShops.values().iterator(); iterator
				.hasNext(); shop.restoreItems())
			shop = iterator.next();
