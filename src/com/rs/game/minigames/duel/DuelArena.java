...
public class DuelArena extends Controler {
	...
	synchronized (this) {
	    ...
	    player.getPackets().sendIComponentText(ifFriendly ? 637 : 631, ifFriendly ? 18 : 40, "" + (target.getSkills().getCombatLevelWithSummoning()));
	...
	if (rules != null && !rules.hasRewardGiven()) {
		if ((victor.isPker && !loser.isPker) || (!victor.isPker && loser.isPker)) {
			victor.sm("A pker cannot duel with an economy player.");
			return;
		}
		...
		victor.getPackets().sendUnlockIComponentOptionSlots(634, 28, 0, 35, 0, 1, 2, 3, 4, 5);
		victor.getPackets().sendInterSetItemsOptionsScript(634, 28, 136, 4, 7, "", "", "", "", "");
		victor.getPackets().sendIComponentText(634, 33, Utils.formatPlayerNameForDisplay(loser.getDisplayName()));
		victor.getPackets().sendIComponentText(634, 22, Integer.toString(loser.getSkills().getCombatLevelWithSummoning()));
		victor.getPackets().sendItems(136, false, loser.getLastDuelRules().getStake().getItems());
	    ...
	    Logger.logDuels(""+player.getUsername()+" completed the duel with " + loser.getUsername() + " previous items are: " + Arrays.toString(victor.getLastDuelRules().getStake().getShiftedItem()) + " new items are " + Arrays.toString(loser.getLastDuelRules().getStake().getShiftedItem()));
	...
	loser.getInterfaceManager().sendInterfaces();
	victor.getInterfaceManager().sendInterfaces();
	WorldTasksManager.schedule(new WorldTask() {
	    @Override
	    public void run() {
