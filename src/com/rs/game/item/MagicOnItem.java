...
public class MagicOnItem {
	...
	public static void processAlchemy(Player player, Item item, boolean low) {
		...
		if (Magic.checkSpellRequirements(player, (low == true ? 21 : 55), true, 554, (low == true ? 3 : 5), 561, 1)) {
			...
			int amount = 1;
			if(player.getPerkHandler().perks.contains(Perk.HIGH_ALCH_PERK))
				amount = player.getInventory().getAmountOf(item.getId()) < 10 ? player.getInventory().getAmountOf(item.getId()): 10;
			player.getInventory().deleteItem(item.getId(), amount);
			player.getSkills().addXp(Skills.MAGIC, (low == true ? 75 : 125));
			if (item.getDefinitions().isNoted())
				item.setId(item.getDefinitions().certId);
			player.getInventory().addItem(995,(EconomyPrices.getAlchPrice(item, low))* amount);
		}
	}
}