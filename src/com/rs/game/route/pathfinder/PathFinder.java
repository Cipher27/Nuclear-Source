...
public class PathFinder {
	...
	private void walk(Entity entity, PathStrategy pathStrategy) {
	  	stepsCount = (calculateRoute(localX,
	  									localY,
											size, pathStrategy,
											null, true, pathBufferX, pathBufferY));
		...
		for(int c = stepsCount-1; c >= 0; c--) {
			if (!entity.addWalkSteps(baseX + routeFinderXArray[c],
					baseY + routeFinderYArray[c], 25,
					true))
				break;
		}
	}
	...
	public static void simpleWalkTo(Entity entity, WorldTile toTile) {
		...
		finder.walk(entity, Routes.createExactStrategy(finder.getLocalX()+moveX, finder.getLocalY()+moveY));
	}
	
	public static void main(String[] args) throws Throwable {
		try {
			Launcher.main(new String[] {});
			//Cache.load(Settings.CACHE_PATH);
		} catch (Exception e) {
			e.printStackTrace();
		}
		//PathFinder finder = new PathFinder(new WorldTile(3220, 3213, 0), 1);
		//finder.walk2(new WorldTile(3220 - 1, 3213 - 1, 0), Routes.createExactStrategy(finder.getLocalX(), finder.getLocalY()+10));
		PathFinder finder = new PathFinder(new WorldTile(5000, 5000, 0), 1);
		finder.walk2(new WorldTile(5010, 5010, 0), Routes.createExactStrategy(finder.getLocalX(), finder.getLocalY()+10));
	}
	...
	public void walk2(WorldTile tile, PathStrategy pathStrategy) {
		...
		if (steps > 25)
			steps = 25;
		for(int c = stepsCount-1; c >= 0; c--)
			System.out.println((baseX + routeFinderXArray[c])+", "+(baseY + routeFinderYArray[c]));
        ClientFrame clientFrame = (new ClientFrame()).open();
        clientFrame.start(anIntArrayArray3155, routeFinderXArray, routeFinderYArray, localX, localY, stepsCount);
	}
	
	public void clipTheRegion(WorldTile tile) {
		for(int x = 1; x < 105; x++)
			for(int y = 1; y < 105; y++) {
				Region region = World.getRegion(new WorldTile(tile.getX()-localX+x, tile.getY()-localY+y, tile.getPlane()).getRegionId());
				if (region.getLoadMapStage() == 0)
					region.quickLoad();
				anIntArrayArray3155[x][y] = World.getMask(tile.getPlane(), tile.getX()-localX+x, tile.getY()-localY+y);
			}
	}
			...
			while_101_: while (i_23_ != i_22_) {
				...
				while_94_: do {
					if (i_18_ > 0) {
						...
						} else if (0 != (anIntArrayArray3155[i_24_ - 1][i_14_ + i_25_ - 1] & 0x4e240000)) {
							if (i_15_ <= 36) {
								/* empty */
							}
						} else {
							for (int i_27_ = 1; i_27_ < i_14_ - 1; i_27_++) {
								if (0 != (anIntArrayArray3155[i_24_ - 1][i_25_ + i_27_] & 0x4fa40000)) {
									if (i_15_ <= 36)
										throw new IllegalStateException();
									break while_94_;
								}
				...
				while_95_: do {
					if (i_18_ < 128 - i_14_) {
						...
						} else if ((anIntArrayArray3155[i_14_ + i_24_][i_14_ + i_25_ - 1] & 0x78240000) != 0) {
							if (i_15_ <= 36) {
								/* empty */
							}
						} else {
							for (int i_28_ = 1; i_28_ < i_14_ - 1; i_28_++) {
								if (0 != (anIntArrayArray3155[i_14_ + i_24_][i_25_ + i_28_] & 0x78e40000)) {
									if (i_15_ <= 36)
										throw new IllegalStateException();
									break while_95_;
								}
				...
				while_97_: do {
					if (i_19_ < 128 - i_14_) {
						...
						} else if (0 != (anIntArrayArray3155[i_24_ + i_14_ - 1][i_25_ + i_14_] & 0x78240000)) {
							if (i_15_ <= 36) {
								/* empty */
							}
						} else {
							for (int i_30_ = 1; i_30_ < i_14_ - 1; i_30_++) {
								if (0 != (anIntArrayArray3155[i_30_ + i_24_][i_14_ + i_25_] & 0x7e240000)) {
									if (i_15_ <= 36)
										throw new IllegalStateException();
									break while_97_;
								}
					...
					if (i_18_ > 0 && i_19_ > 0) {
						if (anIntArrayArray3076[i_18_ - 1][i_19_ - 1] != 0) {
							...
							for (int i_31_ = 1; i_31_ < i_14_; i_31_++) {
								if ((anIntArrayArray3155[i_24_ - 1][i_25_ - 1 + i_31_] & 0x4fa40000) != 0) {
									if (i_15_ <= 36) {
										/* empty */
									}
									break while_98_;
					...
					if (i_18_ < 128 - i_14_ && i_19_ > 0) {
						if (anIntArrayArray3076[1 + i_18_][i_19_ - 1] != 0) {
							...
							for (int i_32_ = 1; i_32_ < i_14_; i_32_++) {
								if (0 != (anIntArrayArray3155[i_24_ + i_14_][i_25_ - 1 + i_32_] & 0x78e40000)) {
									if (i_15_ <= 36)
										throw new IllegalStateException();
									break while_99_;
								}
				...
				while_100_: do {
					if (i_18_ > 0 && i_19_ < 128 - i_14_) {
						...
						} else if (0 != (anIntArrayArray3155[i_24_ - 1][i_14_ + i_25_] & 0x4e240000)) {
							if (i_15_ <= 36) {
								/* empty */
							}
						} else {
							for (int i_33_ = 1; i_33_ < i_14_; i_33_++) {
								if ((anIntArrayArray3155[i_24_ - 1][i_33_ + i_25_] & 0x4fa40000) != 0) {
									if (i_15_ <= 36)
										throw new IllegalStateException();
									break while_100_;
								}
								if (0 != (anIntArrayArray3155[i_33_ + (i_24_ - 1)][i_14_ + i_25_] & 0x7e240000)) {
									if (i_15_ <= 36) {
										/* empty */
									}
									break while_100_;
				...
				if (i_18_ < 128 - i_14_ && i_19_ < 128 - i_14_) {
					...
					} else if ((anIntArrayArray3155[i_14_ + i_24_][i_25_ + i_14_] & 0x78240000) != 0) {
						if (i_15_ <= 36)
							throw new IllegalStateException();
					} else {
						for (int i_34_ = 1; i_34_ < i_14_; i_34_++) {
							if (0 != (anIntArrayArray3155[i_24_ + i_34_][i_25_ + i_14_] & 0x7e240000)) {
								if (i_15_ <= 36) {
									/* empty */
								}
								continue while_101_;
							}
							if (0 != (anIntArrayArray3155[i_24_ + i_14_][i_34_ + i_25_] & 0x78e40000)) {
								if (i_15_ <= 36) {
									/* empty */
								}
								continue while_101_;
	...
	final boolean method4006(int i, int i_6_, PathStrategy class336,
		...
		try {
			...
			while (i_15_ != i_14_) {
				...
				if (i_10_ > 0
						&& i_14_ > -1//added
						&& (i_16_ - 1) > -1 && i_17_ > -1//added
						&& (i_16_ - 1) < 110 && i_17_ < 110//added
				...
				if (i_11_ > 0
						&& i_14_ > -1//added
						&& i_16_ > -1 && (i_17_ - 1) > -1//added
						&& i_16_ < 110 &&  (i_17_ - 1) < 110//added
				...
				if (i_10_ > 0
						&& i_11_ > 0
						&& i_14_ > -1//added
						&& (i_16_ - 1) > -1 && (i_17_ - 1) > -1//added
						&& (i_16_) < 110 &&  (i_17_) < 110//added
				...
				if (i_10_ < 127
						&& i_11_ < 127
						&& i_14_ > -1//added
						&& (i_16_) > -1 && (i_17_) > -1//added
						&& (i_16_ + 1) < 110 &&  (i_17_ + 1) < 110//added
	...
	public int calculateRoute(int startX, int startY, int size, PathStrategy pathStrategy,
		...
		try {
			...
			boolean bool_6_;
			if (size == 1) {
				bool_6_ = method4006(startX, startY, pathStrategy, planeClipData,
						-369838027);
			} else if (2 == size)
				bool_6_ = method2120(startX, startY, pathStrategy, planeClipData,
						-1327541421);
			else
				bool_6_ = Class372.method4593(startX, startY, size, pathStrategy,
						planeClipData, (byte) 58);
			int i_7_ = startX - 64;
			int i_8_ = startY - 64;
			...
			int i_27_;
			int i_26_ = (i_27_ = anIntArrayArray3076[i_9_ - i_7_][i_10_
					- i_8_]);
