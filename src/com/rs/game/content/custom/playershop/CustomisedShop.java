package com.rs.game.content.custom.playershop;
...
public class CustomisedShop implements Serializable {
	...
	public static void searchByName(Player searcher, String itemName) {
		...
		for (Player player : allPlayers) {
			if (player.getPlayerShop() == null)
				continue;
			for (MyShopItem item : player.getPlayerShop().getShopItems()) {
				if (item == null
						|| item.getItem() == null
	...
	public void sendOwnShop() {
		...
		player.getSkills();
		if (Skills.getTotalLevel(player) < 400) {
	...
	public boolean addItem(Item itemToAdd) {
		...
		MyShopItem item = new MyShopItem(itemToAdd, (itemToAdd).getDefinitions().getValue());
	...
	public boolean remove(Player buyer, MyShopItem item) {
		...
		if (buyer != null) {
			...
			if (player.getPlayerShop().isLocked()) {
		...
		if (removed) {
			if (buyer == null) {
 			...
 			buyer.getInventory().removeItemMoneyPouch(995,price);
