...
public class GodwarsInstanceBoss extends NPC {

	public GodwarsInstanceBoss(int id, WorldTile tile, int mapAreaNameHash,
			boolean canBeAttackFromOutOfArea, boolean spawned) {
		super(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, spawned);
	}
	
	public final ArrayList<NPC> minions = new ArrayList<NPC>(3);
		...
		for (final int regionId : getMapRegionsIds()) {
			...
			if (npcIndexes != null) {
				for (final int npcIndex : npcIndexes) {
					...
					if (npc == null || npc == this || npc instanceof Pet || npc.hasFinished() || npc.isDead() || !npc.withinDistance(this, 64)
						...
						continue;
					if (!possibleTarget.contains(npc))
			...
			public void run() {
				...
				if (minions != null && !minions.isEmpty()) {
					for (NPC minion : minions) {
						if (minion == null || !(minion instanceof GodWarMinion))
							continue;
						if (minion.hasFinished() || minion.isDead())
							((GodWarMinion)minion).respawn();
					}
				}
			}
		}, getCombatDefinitions().getRespawnDelay() * getRespawnDelay(),//600,
