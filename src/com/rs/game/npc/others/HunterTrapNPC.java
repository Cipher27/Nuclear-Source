	...
	public void processNPC() {
		...
		if (captureTicks > 0) {
			...
			if (captureTicks == 5) {
				...
				if (!OwnedObjectManager.convertIntoObject(o, new WorldObject(hNPC.getSuccesId(), o.getType(), o.getRotation(), new WorldTile(o.getTileHash())), new ConvertEvent() {
					...
					public boolean canConvert(final Player player) {
						...
						double ratio = ((double) (trap.getRequirementLevel() + 15) / lureLevel) * currentLevel;
						if (currentLevel < lureLevel || ratio < Utils.random(95))
							return false;
						return true;
					}
				})) {
					int anim = hNPC.getIds()[0];
					if (anim != -1)
