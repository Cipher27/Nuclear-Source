...
public class KalGerWarmonger extends DungeonBoss {
 ...
 /*   @Override
	...
	setNextForceTalk(new ForceTalk("IMPOSSIBRU!"));
	...
	if (typeTicks == 1) {
	    ...
	    setNextForceMovement(new NewForceMovement(this, 1, nextFlyTile, 5,
		    Utils.getAngle(nextFlyTile.getX() - getX(), nextFlyTile.getY() - getY())));
	} else if (typeTicks == 6) {
	    setNextGraphics(new Graphics(2870));
	...
	for (Player player : getManager().getParty().getTeam()) {
		...
		continue;
	    player.getPackets()
		    .sendGameMessage("The Warmonger steals your " + (usingPiety ? "Piety" : "Turmoil") + " effects!");
	    stolenEffects = true;
	}
    }
