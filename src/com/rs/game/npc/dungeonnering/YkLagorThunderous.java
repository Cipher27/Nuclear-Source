...
public class YkLagorThunderous extends DungeonBoss {
    ...
    private static int[][] BROKEN_FLOORS_TILES = { { 5, 11 }, { 2, 13 }, { 10, 11 }, { 13, 12 }, { 11, 8 }, { 11, 6 },
	    { 13, 4 }, { 9, 5 }, { 10, 3 }, { 3, 3 }, { 4, 9 }, { 7, 4 }, { 4, 6 } };
	...
	mysteriousMages[index++] = new YkLagorMage(this, 11887, getManager().getTile(getReference(), 7, 3),
		getManager(), 1);
	mysteriousMages[index++] = new YkLagorMage(this, 11887, getManager().getTile(getReference(), 4, 5),
		getManager(), 1);
	mysteriousMages[index++] = new YkLagorMage(this, 11887, getManager().getTile(getReference(), 3, 8),
		getManager(), 1);
	mysteriousMages[index++] = new YkLagorMage(this, 11887, getManager().getTile(getReference(), 4, 11),
		getManager(), 1);
	mysteriousMages[index++] = new YkLagorMage(this, 11887, getManager().getTile(getReference(), 7, 12),
		getManager(), 1);
	mysteriousMages[index++] = new YkLagorMage(this, 11887, getManager().getTile(getReference(), 10, 11),
		getManager(), 1);
	mysteriousMages[index++] = new YkLagorMage(this, 11887, getManager().getTile(getReference(), 11, 8),
		getManager(), 1);
	mysteriousMages[index++] = new YkLagorMage(this, 11887, getManager().getTile(getReference(), 10, 5),
		getManager(), 1);
    }

    private final static String[] QUOTES = { "We will break you!", "You've outlived your use!",
	    "You do not belong here.", "Your soul belongs to us!", "Mah zodas'bakh me'ah." };
     ...
     * { ArrayList<Entity> possibleTargets = super.getPossibleTargets(npc,
     * true); if (special) { for (int index = 0; index < PILLAR_SAFEZONE.length;
     * index++) { WorldTile tile = getManager().getTile(getReference(),
     * PILLAR_SAFEZONE[index][0], PILLAR_SAFEZONE[index][1]); for (Entity t :
     * possibleTargets) { if (t.getX() == tile.getX() && t.getY() ==
     * tile.getY()) possibleTargets.remove(t); } } } return possibleTargets; }
     */

    public void sendBrokenFloor() {
	for (int index = 0; index < BROKEN_FLOORS_TILES.length; index++) {
	    WorldTile tile = getManager().getTile(getReference(), BROKEN_FLOORS_TILES[index][0],
		    BROKEN_FLOORS_TILES[index][1]);
