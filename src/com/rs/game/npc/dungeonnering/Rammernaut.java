...
public class Rammernaut extends DungeonBoss {
	...
	if (entity instanceof Player) {
		...
		public void run() {
			...
			for (int distance = 6; distance >= 0; distance--) {
			    tile = new WorldTile(new WorldTile(entity.getX() + (dirs[0] * distance),
				    entity.getY() + (dirs[1] * distance), entity.getPlane()));
	...
	if (chargeTarget != null) {
		...
		if (count != 3 && !World.isTileFree(getPlane(), getX(), getY(), getSize()))
		    fail();
		else if (Utils.isOnRange(getX(), getY(), getSize(), chargeTarget.getX(), chargeTarget.getY(),
	 ...
	 * return defs.getAttackDelay(); } } // System.out.println(
	 * "Time to charge " + chargeCount); return 0; }
	 */
    }
