...
public final class Stomp extends DungeonBoss {
	...
	if (getId() == 9781)
		...
		if (count == IVULNERABLE_TIMER - 3) {
		    ...
		    for (int[] s : shadows) {
			WorldObject object = getManager().spawnObjectTemporary(getReference(), 49268, 10, 0, s[0], s[1],
				30000);
			for (Entity target : possibleTargets)
			    if (target.getX() == object.getX() && target.getY() == object.getY())
				target.applyHit(new Hit(this, 1 + Utils.random((int) (target.getMaxHitpoints() * 0.8)),
	...
	int id = count == IVULNERABLE_TIMER ? (49274 + lodeStoneType * 2 + index)
		: lodestones[index] ? lodeStoneType == 0 ? 51099 : lodeStoneType == 1 ? 51601 : 51647 : (49270 + index);
	...
	if (player.getInventory().containsItem(CRYSTAL[lodeStoneType], 1)) {
	    ...
	    player.getPackets()
		    .sendGameMessage("You need a "
			    + ItemDefinitions.getItemDefinitions(CRYSTAL[lodeStoneType]).getName().toLowerCase()
			    + " to activate this lodestone.");
	...
	setNextGraphics(new Graphics(2407));
	setCantInteract(true);
	for (Entity target : getPossibleTargets()) {
	    if (target instanceof Player) {
		Player player = (Player) target;
		player.getPackets().sendGameMessage(
			"Stomp enters a defensive stance. It is currently invulnerable, but no longer protecting the portal's lodestones!");
	    }
	}
	destroyExistingDebris();
	...
	WorldTasksManager.schedule(new WorldTask() {
		...
		if (loop == 0) {
			...
			((Player) source).getControlerManager().processNPCDeath(Stomp.this);
		    drop();
		    reset();
		    setCantInteract(true);
    ...
    /*
     * @Override public Item sendDrop(Player player, Drop drop) { Item item =
     * new Item(drop.getItemId());
     * player.getInventory().addItemDrop(item.getId(), item.getAmount()); return
     * item; }
     */
