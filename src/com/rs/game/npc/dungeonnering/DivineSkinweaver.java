...
public final class DivineSkinweaver extends DungeonBoss {
    ...
    private static final String[] CLOSE_HOLE_MESSAGES = { "Ride the wind and smite the tunnel.",
	    "We have little time, tear down the tunnel.", "Churra! Bring down the tunnel while you can." };
	...
	for (int[] hole : HOLES) {
		...
		holes.add(new int[] { object.getX() + Utils.ROTATION_DIR_X[object.getRotation()],
			object.getY() + Utils.ROTATION_DIR_Y[object.getRotation()] });
	}
	if (holes.size() == 0)
	    return null;
	...
	if (respawnDelay > 0) {
	    ...
	    if (coords != null) {
		skeletons.add((DungeonSkeletonBoss) getManager().spawnNPC(SKELETONS[Utils.random(SKELETONS.length)], 0,
			new WorldTile(coords[0], coords[1], 0), getReference(), DungeonConstants.BOSS_NPC, 0.4D));
		respawnDelay = 10;
	    }
	}
	if (healDelay > 0) {
	...
	if (count < holeClosed.length || skeletons.size() > 0) {
	    player.getDialogueManager().startDialogue("SimpleNPCMessage", getId(),
		    "Chat later and kill the skeletons now, brah.");
	    return;
	}
	...
	if (!requestedClose) {
	    player.getPackets()
		    .sendGameMessage("The portal is fully powered and shocks you with a large burst of energy.");
