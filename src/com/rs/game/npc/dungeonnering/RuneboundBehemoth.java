...
public class RuneboundBehemoth extends DungeonBoss {
	...
	if ((hit.getLook() == HitLook.MELEE_DAMAGE && artifacts[0].isPrayerEnabled())
		|| (hit.getLook() == HitLook.RANGE_DAMAGE && artifacts[1].isPrayerEnabled())
		|| (hit.getLook() == HitLook.MAGIC_DAMAGE && artifacts[2].isPrayerEnabled()))
	    return;
	hit.setDamage(0);
    }
	...
	public void setActive(boolean active, boolean message) {
		...
		for (Player p2 : getManager().getParty().getTeam()) {
			...
			continue;
		    p2.getPackets().sendGameMessage("The " + ARTIFACT_TYPE[type] + " artifact has been "
