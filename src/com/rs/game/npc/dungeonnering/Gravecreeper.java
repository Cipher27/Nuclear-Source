...
public class Gravecreeper extends DungeonBoss {
	...
	for (BurnTile bTile : burnedTiles) {
		...
		tileLoop: for (WorldTile tile : bTile.tiles) {
			...
			continue tileLoop;
		    p2.applyHit(new Hit(this, (int) Utils.random(getMaxHit() * .1, getMaxHit() * .25),
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				if (getManager().isDestroyed())
				    return;
				setNextWorldTile(getManager().getTile(getReference(), 3 + Utils.random(4) * 3,
					3 + Utils.random(4) * 3));
	...
	for (int x = 0; x < plinths.length; x++) {
		...
		if (plinths[x][y] == null)
		    continue;
		WorldTile altarLoc = getManager().getTile(getReference(), TOMB_LOC_POS_2[y][x][0],
			TOMB_LOC_POS_2[y][x][1]);
		...
		if (!triggeredPlinths[x][y]) {
			...
			if (Utils.isOnRange(t.getX(), t.getY(), t.getSize(), altarLoc.getX(), altarLoc.getY(), 1, 2))
			    t.applyHit(new Hit(this, Utils.random((int) (t.getMaxHitpoints() * 0.1)) + 1,
    ...
    private static final int[][][] TOMB_LOC_POS_2 = { { { 2, 2 }, { 6, 2 }, { 9, 2 }, { 13, 2 } },
	    { { 2, 6 }, { 7, 7 }, { 8, 7 }, { 13, 6 } }, { { 2, 9 }, { 7, 8 }, { 8, 8 }, { 13, 9 } },
	...
	for (int x = 0; x < plinths.length; x++) {
		...
		if (plinths[x][y] == null)
		    continue;
		WorldObject altar = getManager().getObjectWithType(getReference(), 10, TOMB_LOC_POS_2[y][x][0],
			TOMB_LOC_POS_2[y][x][1]);
		if (altar == null)
		    continue;
