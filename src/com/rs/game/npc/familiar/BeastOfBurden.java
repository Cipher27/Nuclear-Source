...
public class BeastOfBurden implements Serializable {
    ...
    public void setBeastItems(Item[] items) {
    	beastItems.setItems(items);
    }
    ...
    public void addItem(int slot, int amount) {
        ...
        if (item == null)
            return;
        if (!ItemConstants.isTradeable(item)|| item.getId() == 4049
                || (familiar.canStoreEssOnly() && item.getId() != 1436 && item
                .getId() != 7936)
                || item.getDefinitions().getValue() > 50000
                || item.getDefinitions().isLended() || item.getId() == 995  || item.getId() == 744 ) {
    ...
    public void dropBob() {
		...
		for (int i = 0; i < beastItems.getSize(); i++) { 
			...
			if (item != null) {
				if (player.getRights() < 2 || player.getRights() >= 2)
