...
public abstract class Familiar extends NPC implements Serializable {
	...
	public Familiar(Player owner, Pouches pouch, WorldTile tile, int mapAreaNameHash, boolean canBeAttackFromOutOfArea) {
		super(Summoning.getNPCId(pouch.getRealPouchId(),owner), tile, mapAreaNameHash, canBeAttackFromOutOfArea, false);
		this.owner = owner;
		this.pouch = pouch;
		resetTickets();
		//this.write();
		specialEnergy = 60;
		if(owner.getNpcOverrideId() != 0){
			NPCDefinitions defs = NPCDefinitions.getNPCDefinitions(owner.originalId);
			this.setName(""+defs.getName());
		}
		if (getBOBSize() > 0)
	...
	public int getOriginalId() {
		return Summoning.getNPCId(pouch.getRealPouchId(), owner);
	}
	...
	public void processNPC() {
		...
		if (trackTimer ==  (owner.getPerkHandler().perks.contains(Perk.POUCH_PERK)?75:50)) { //for the increased pouch duration xD
			trackTimer = 0;
			ticks--;
			if (trackDrain)
				owner.getSkills().drainSummoning(1);
			trackDrain = !trackDrain;
			NPC npc = owner.getFamiliar();
			for (FamiliarForceTalk forceTalk : FamiliarForceTalk.values()) {
				if (forceTalk.getId() == npc.getId() || forceTalk.getId2() == npc.getId()) {
					owner.getFamiliar().setNextForceTalk(
							new ForceTalk(forceTalk.getWord()[Utils.getRandom(forceTalk.getWord().length - 1)]));
				}
			}
			if (ticks == 2)
		...
		if (owner.isCanPvp() && getId() == getOriginalId()) {
			...
			call(false);
			return;
		} else if (!owner.isCanPvp() && getId() == originalId && pouch != Pouches.MAGPIE && pouch != Pouches.IBIS
				&& pouch != Pouches.BEAVER && pouch != Pouches.MACAW && pouch != Pouches.FRUIT_BAT) {
		...
		if (owner.getAttackedBy() != null) {
			if (canAttack(owner.getAttackedBy()) & owner.familiarAutoAttack) {
				setTarget(owner.getAttackedBy());
			} else {
				setTarget(owner.getAttackedBy());
			}
		} else if (!owner.familiarAutoAttack) {
	...
	public void storeScrolls(int itemId) {
		...
		if (itemId == Summoning.getScrollId(pouch.getRealPouchId())) {
			...
			owner.setNextAnimation(new Animation(1649));
			owner.getDialogueManager().startDialogue("SimpleMessage",
	...
	public boolean hasSpecialActivated() {
		if (owner.getTemporaryAttributtes().get("FamiliarSpec") != null)
			return true;
		return false;
	}
