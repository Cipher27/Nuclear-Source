...
public abstract class CombatScript {
	...
	public void loadSkillDrop(final Player target) {
		WorldTasksManager.schedule(new WorldTask() {
		 @Override
			public void run() {
				...
				if (skill == Skills.PRAYER)
					target.getPrayer().drainPrayer(150);
				else {
					int lvl = target.getSkills().getLevel(skill);
					lvl -= 1 + Utils.getRandom(4);
					target.getSkills().set(skill, lvl < 0 ? 0 : lvl);
				}
				target.sm("Your " + Skills.SKILL_NAME[skill]
						+ " has been dropped!");
			}
