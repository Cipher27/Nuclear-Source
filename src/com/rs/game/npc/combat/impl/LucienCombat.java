...
public class LucienCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
		...
		int attackStyle = Utils.getRandom(12);
		...
		if (attackStyle == 0) { // normal mage move
			...
			delayHit(
					...
					getRangeHit(
							npc,
							getRandomMaxHit(npc, Utils.random(100,500),
			...
			delayHit(
					...
					getMagicHit(
							npc,
							getRandomMaxHit(npc, defs.getMaxHit() -100,
									NPCCombatDefinitions.MAGE, target)));
			...
			delayHit(
					...
					2,
					target,
					getRegularHit(npc, target.getMaxHitpoints()> 900 ? Utils.random(400,600)
							: target.getMaxHitpoints() - 500));
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					for (Entity t : npc.getPossibleTargets()) {
						...
						t.applyHit(new Hit(npc, t.getHitpoints() -10, HitLook.REGULAR_DAMAGE, 0));
						}
					}
