...
public class BulwarkBeastCombat extends CombatScript {
	...
	if (Utils.random(15) == 0) {
		...
		if (Utils.isOnRange(t.getX(), t.getY(), t.getSize(), npc.getX(), npc.getY(), npc.getSize(), 0)) {
		    ...
		    delayHit(npc, 1, t, getRegularHit(npc,
			    1 + Utils.random((int) (npc.getMaxHit(NPCCombatDefinitions.MELEE) * 0.7))));
		}
	    }
	...
	// mage, range, melee
	int attackStyle = Utils.random(Utils.isOnRange(target.getX(), target.getY(), target.getSize(), npc.getX(),
		npc.getY(), npc.getSize(), 0) ? 3 : 2);
	switch (attackStyle) {
	case 0:
	...
	case 1:
		...
		delayHit(npc, 1, t, getRangeHit(npc, getRandomMaxHit(npc, NPCCombatDefinitions.RANGE, attackStyle, t)));
	    }
	    break;
	case 2:
	    npc.setNextAnimation(new Animation(defs.getAttackEmote()));
	    delayHit(npc, 0, target, getMeleeHit(npc, getRandomMaxHit(npc, NPCCombatDefinitions.MELEE, attackStyle, target)));
	    break;
	}
	return npc.getAttackSpeed();
    }
