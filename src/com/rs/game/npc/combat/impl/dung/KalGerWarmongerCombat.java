...
public class KalGerWarmongerCombat extends CombatScript {
	...
	if (boss.isUsingMelee()) {
	    ...
	    } else if (!Utils.isOnRange(npc.getX(), npc.getY(), npc.getSize(), target.getX(), target.getY(),
		    target.getSize(), 0))
		return 0;
	}
	boss.setPullTicks(0);
	...
	if (boss.getType() == 1) {// NO WEAPONS HUR
	    ...
	    delayHit(npc, 0, target,
		    getMeleeHit(npc, getRandomMaxHit(npc, boss.getMaxHit(), NPCCombatDefinitions.MELEE, target)));
