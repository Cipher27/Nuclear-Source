...
public class UnholyCursebearerCombat extends CombatScript {
	...
	int attackStyle = Utils.isOnRange(target.getX(), target.getY(), target.getSize(), npc.getX(), npc.getY(),
		npc.getSize(), 0) ? Utils.random(2) : 0;
	if (target instanceof Player && target.getTemporaryAttributtes().get(Key.UNHOLY_CURSEBEARER_ROT) == null) {
		...
		@Override
		public void run() {
		    Integer value = (Integer) target.getTemporaryAttributtes().get(Key.UNHOLY_CURSEBEARER_ROT);
		    if (player.hasFinished() || npc.hasFinished()
			    || !((DungeonBoss) npc).getManager().isAtBossRoom(player) || value == null) {
			target.getTemporaryAttributtes().remove(Key.UNHOLY_CURSEBEARER_ROT);
			stop();
			return;
		    }
		    int damage = 20 * value;
		    ...
		    target.getTemporaryAttributtes().put(Key.UNHOLY_CURSEBEARER_ROT, value + 1);
		}
	...
	case 0:
		...
		for (Entity t : npc.getPossibleTargets()) {
		    ...
		    delayHit(npc, 1, t, getMagicHit(npc, getRandomMaxHit(npc,
			    (int) (npc.getMaxHit(NPCCombatDefinitions.MAGE) * 0.6), NPCCombatDefinitions.MAGE, t)));
		}
	    } else {
