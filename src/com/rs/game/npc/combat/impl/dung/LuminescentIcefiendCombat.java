...
public class LuminescentIcefiendCombat extends CombatScript {
	...
	if (magicAttack) {
	    ...
	    delayHit(icefiend, 2, target,
		    getMagicHit(npc, getRandomMaxHit(npc, icefiend.getMaxHit(), NPCCombatDefinitions.MAGE, target)));
	} else {
	    npc.setNextAnimation(new Animation(13337));
	    World.sendProjectile(npc, target, 2530, 30, 16, 35, 35, 0, 0);
	    delayHit(icefiend, 2, target, getRangeHit(npc,
		    getRandomMaxHit(npc, (int) (icefiend.getMaxHit() * .90), NPCCombatDefinitions.RANGE, target)));
