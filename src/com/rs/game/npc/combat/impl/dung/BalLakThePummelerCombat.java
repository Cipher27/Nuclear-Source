...
public class BalLakThePummelerCombat extends CombatScript {
	...
	if (Utils.random(5) == 0) {
		...
		int damage2 = getRandomMaxHit(npc, npc.getMaxHit(), NPCCombatDefinitions.MELEE, t);
		if (t instanceof Player) {
		    Player player = (Player) t;
		    if ((damage > 0 || damage2 > 0)) {
			player.getPackets()
				.sendGameMessage("You are injured and currently cannot use protection prayers.");
		    }
		}
	...
	case 0:// reg melee left
	    ...
	    delayHit(npc, 0, target, getMeleeHit(npc, getRandomMaxHit(npc,
		    (int) (npc.getMaxHit(NPCCombatDefinitions.MELEE) * 0.8), NPCCombatDefinitions.MELEE, target)));
	    delayHit(npc, 2, target, getMeleeHit(npc, getRandomMaxHit(npc,
		    (int) (npc.getMaxHit(NPCCombatDefinitions.MELEE) * 0.8), NPCCombatDefinitions.MELEE, target)));
	    WorldTasksManager.schedule(new WorldTask() {
	...
	case 1:// magic attack multi
		...
		delayHit(npc, 1, t,
			getMagicHit(npc, getRandomMaxHit(npc, (int) (boss.getMaxHit() * 0.6), NPCCombatDefinitions.MAGE, t)));
	    }
