...
public class GeomancerCombat extends CombatScript {
	...
	if (Utils.random(boss.getManager().getParty().getTeam().size() > 1 ? 6 : 8) == 0 && !boss.isCantInteract()) {
	    ...
	    if (room != null && room.getRoom() == DungeonUtils.getBossRoomWithChunk(DungeonConstants.ABANDONED_FLOORS,
		    24, 640)) {
	...
	for (Entity t : npc.getPossibleTargets()) {
	    ...
	    delayHit(npc, 1, t, getMagicHit(npc, getRandomMaxHit(npc, (int) (npc.getMaxHit(NPCCombatDefinitions.MAGE) * .7),
	...
	int damage = getRandomMaxHit(npc, (int) (npc.getMaxHit(NPCCombatDefinitions.MAGE) * 0.95), NPCCombatDefinitions.MAGE,
		target);
