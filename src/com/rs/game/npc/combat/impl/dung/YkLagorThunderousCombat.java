...
public class YkLagorThunderousCombat extends CombatScript {
	...
	if (boss.getNextAttack() < 2) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			if (cycles == 3) {
				...
				if (targets.contains(player)) {
				    ...
				    player.getPackets()
					    .sendGameMessage("Yk'Lagor sees you and pulls you closer, energising him.");
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			if (cycles == 2) {
				...
				t.applyHit(
					new Hit(boss, Utils.random(t.getMaxHitpoints()) + 1, HitLook.REGULAR_DAMAGE));
	...
	player.setNextForceMovement(new NewForceMovement(player, 0, tile, 2,
		Utils.getAngle(tile.getX() - player.getX(), tile.getY() - player.getY())));
	WorldTasksManager.schedule(new WorldTask() {
		...
		if (disablePrayers) {
		    player.getPackets().sendGameMessage("You've been injured and you cannot use protective "
			    + (player.getPrayer().isAncientCurses() ? "curses" : "protective prayers") + "!");
		}
	    }
	}, 1);
    }
