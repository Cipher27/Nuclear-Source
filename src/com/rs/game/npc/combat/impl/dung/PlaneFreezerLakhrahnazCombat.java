...
public class PlaneFreezerLakhrahnazCombat extends CombatScript {
	...
	if (Utils.random(3) == 0) {
	    ...
	    if (attackStyle == 1 && !Utils.isOnRange(target.getX(), target.getY(), target.getSize(), npc.getX(),
		    npc.getY(), npc.getSize(), 0))
		attackStyle = 0;
	    switch (attackStyle) {
	    case 0:
		...
		for (Entity t : npc.getPossibleTargets()) {
		    ...
		    delayHit(npc, 1, t, getMagicHit(npc, getRandomMaxHit(npc, npc.getMaxHit(), NPCCombatDefinitions.MAGE, target)));
		}
		break;
	    case 1:
