...
public class NightGazerKhighorahkCombat extends CombatScript {
	...
	for (Entity target : gazer.getPossibleTargets()) {
	    ...
	    delayHit(gazer, 1, target, getRangeHit(gazer,
		    getRandomMaxHit(gazer, (int) (gazer.getMaxHit() * 0.6), NPCCombatDefinitions.RANGE, target)));
	}
	...
	if (!gazer.isUsedSpecial()) {
		...
		if (Utils.isOnRange(npc.getX(), npc.getY(), npc.getSize(), t.getX(), t.getY(), t.getSize(), 1)) {
		    ...
		    npc.setNextGraphics(
			    new Graphics(/* gazer.isSecondStage() ? 2391 : */2390));
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			if (ticks == 1) {
			    ...
			    for (Entity t : targets) {
				if (Utils.isOnRange(npc.getX(), npc.getY(), npc.getSize(), t.getX(), t.getY(),
					t.getSize(), 1)) {
					...
					tile = new WorldTile(new WorldTile(t.getX() + (dirs[0] * distance),
						t.getY() + (dirs[1] * distance), t.getPlane()));
