...
public class ShadowForgerIhlakhizanCombat extends CombatScript {
	...
	for (Entity t : npc.getPossibleTargets())
	    t.getTemporaryAttributtes().remove(Key.SHADOW_FORGER_SHADOW);
	...
	if (Utils.random(4) == 0) {
		...
		for (int i = 0; i < 10; i++) {
		    final WorldTile tile = ((ShadowForgerIhlakhizan) npc).getManager().getTile(forger.getReference(),
			    2 + Utils.random(12), 2 + Utils.random(12));
			...
			public void run() {
				...
				if (player.isDead() || player.getX() != tile.getX() || player.getY() != tile.getY())
				    continue;
				player.applyHit(
					new Hit(npc, Utils.random(npc.getMaxHit(NPCCombatDefinitions.RANGE)) + 1,
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			for (Entity t : npc.getPossibleTargets()) {
			    t.applyHit(new Hit(npc, Utils.random((int) (t.getMaxHitpoints() * 0.74)) + 1,
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					if (ticks == 1) {
						...
						tile = new WorldTile(new WorldTile(target.getX() + (dirs[0] * distance),
							target.getY() + (dirs[1] * distance), target.getPlane()));
					    ...
					    target.setNextForceMovement(
						    new NewForceMovement(target, 0, tile, 2, target.getDirection()));
					} else if (ticks == 2) {
	...
	int attackStyle = Utils.random(Utils.isOnRange(npc.getX(), npc.getY(), npc.getSize(), target.getX(),
		target.getY(), target.getSize(), 0) ? 2 : 1);
