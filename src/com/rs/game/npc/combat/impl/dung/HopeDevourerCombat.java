...
public class HopeDevourerCombat extends CombatScript {
	...
	if (Utils.random(10) == 0) {
		...
		public void run() {
			...
			int damage = (int) Utils.random(npc.getMaxHit(NPCCombatDefinitions.MAGE) * .85,
	...
	if (Utils.random(5) == 0) {
	    ...
	    final int damage = (int) Utils.random(npc.getMaxHit(NPCCombatDefinitions.MELEE) * .85,
		    npc.getMaxHit(NPCCombatDefinitions.MELEE));
	    if (target instanceof Player) {
		Player player = (Player) target;
		player.getSkills().set(Skills.DEFENCE,
			(int) (player.getSkills().getLevel(Skills.DEFENCE) - (damage * .05)));
	    }
	    ...
	    int damage = (int) Utils.random(npc.getMaxHit(NPCCombatDefinitions.MELEE) * .75,
		    npc.getMaxHit(NPCCombatDefinitions.MELEE));
	    if (target instanceof Player) {
		Player player = (Player) target;
