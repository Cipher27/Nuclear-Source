...
public class RuneboundBehemothCombat extends CombatScript {
		...
		public void run() {
			...
			for (Entity t : boss.getPossibleTargets()) {
				...
				if (t.getX() != tile.getX() || t.getY() != tile.getY())
				    continue tileLoop;
				t.applyHit(new Hit(boss, (int) Utils.random(boss.getMaxHit() * .6, boss.getMaxHit()),
	...
	boolean distanced = !Utils.isOnRange(npc.getX(), npc.getY(), npc.getSize(), target.getX(), target.getY(),
		target.getSize(), 0);
