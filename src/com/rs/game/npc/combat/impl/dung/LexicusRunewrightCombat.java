...
public class LexicusRunewrightCombat extends CombatScript {
	...
	int attack = Utils.random(Utils.isOnRange(target.getX(), target.getY(), target.getSize(), npc.getX(),
		npc.getY(), npc.getSize(), 0) ? 5 : 4);
	switch (attack) {
	case 0:// Range
	case 1:
	case 2:
	...
	WorldTasksManager.schedule(new WorldTask() {
		...
		if (cycle == 1) {
			...
			tileLoop: for (WorldTile tile : targets) {
				...
				continue tileLoop;
			    entity.applyHit(new Hit(npc, (int) (entity instanceof Familiar ? 1000
				    : Utils.random(entity.getMaxHitpoints() * .6, entity.getMaxHitpoints() * .9)),
