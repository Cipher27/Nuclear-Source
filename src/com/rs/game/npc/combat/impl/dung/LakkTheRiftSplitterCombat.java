...
public class LakkTheRiftSplitterCombat extends CombatScript {
    ...
    private static final String[] MESSAGES = { "A flame portal will flush you out!", "Taste miasma!",
	...
	for (Player player : manager.getParty().getTeam()) {
		...
		smash = true;
		delayHit(npc, 0, player, getRegularHit(npc, getRandomMaxHit(npc,
			(int) (npc.getMaxHit(NPCCombatDefinitions.MELEE) * .85), NPCCombatDefinitions.MELEE, player)));
		delayHit(npc, 0, player, getRegularHit(npc, getRandomMaxHit(npc,
			(int) (npc.getMaxHit(NPCCombatDefinitions.MELEE) * .60), NPCCombatDefinitions.MELEE, player)));
	    }
	}
	if (smash) {
	...
	// melee or magic
	boolean onRange = Utils.isOnRange(npc.getX(), npc.getY(), npc.getSize(), target.getX(), target.getY(),
		target.getSize(), 0);
