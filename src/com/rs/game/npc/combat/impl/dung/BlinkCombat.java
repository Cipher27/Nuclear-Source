...
public class BlinkCombat extends CombatScript {
	 ...
	 //   boss.playSoundEffect(2989);
		...
		public void run() {
		  ...
		  //  boss.playSoundEffect(3002);
			...
			delayHit(boss, 0, t, new Hit(boss, (int) Utils.random(boss.getMaxHit() * .6D, boss.getMaxHit()),
	...
	boolean atDistance = !Utils.isOnRange(npc.getX(), npc.getY(), npc.getSize(), target.getX(), target.getY(),
		target.getSize(), 0);
	if (atDistance || Utils.random(3) == 0) {
		...
		if (manager.getParty().getTeam().size() > 1 || Utils.random(3) == 0) {
			...
			public void run() {
				...
				for (WorldTile tile : knifeTargets) {
				 ...
				 //   int delay = Utils.projectileTimeToCycles(World.sendProjectile(boss, boss.getNextPath(), tile, 2853, 18, 18, 50, 50, 0, 0).getEndTime());
					...
					if (!t.matches(tile))
					    continue entityLoop;
					delayHit(boss, 12, t, getRangeHit(boss,
						getRandomMaxHit(boss, boss.getMaxHit(), NPCCombatDefinitions.RANGE, t)));
				    }
				}
				stop();
				return;
		    ...
		    delayHit(boss, 1, target,
			    getRangeHit(boss, getRandomMaxHit(boss, boss.getMaxHit(), NPCCombatDefinitions.RANGE, target)));
		}
	    } else {
		if (Utils.random(7) == 0) {
	    ...
	    delayHit(boss, 0, target,
		    getMeleeHit(boss, getRandomMaxHit(boss, boss.getMaxHit(), NPCCombatDefinitions.MELEE, target)));
	    return 4;
	}
    }
}
