...
public class AchievementManager implements Serializable {
	...
	public void notifyUpdate(Class<?> clazz) {
		...
		if (!completeAchievement(achievement)) {
			...
			if (achievement.isComplete(player)) {
				...
				player.getInterfaceManager().doAchievementInterface(""+achievement.getTitle());
	...
	public void sendNotification(String message) {
		...
		if(player.showAchievementMessage){
	...
	public void displayAchievements(Types type) {
		...
		if (!hasUnlocked(type)) {
			...
			player.getDialogueManager().startDialogue("SimpleMessage", "You have not yet unlocked this achievement type.", "You need a total level of " + Utils.formatNumber(total) + " to unlock " + type.name().toLowerCase() + " achievements");
			player.sendMessage("You need a total level of " + Utils.formatNumber(total) + " to unlock " + type.name().toLowerCase() + " achievements");
		}
	...
	/**
	 * sends the custom interface
	 */
	public Types selectedType = null;
	public void sendTestInterface(Types type){
		player.getInterfaceManager().sendInterface(3043);
		sendEmpty();
		for(int i = 1; i < 24; i ++)
			player.getPackets().sendIComponentText(3043, i, ""); 
		Iterator<Entry<String, Achievement>> it = getAchievements().entrySet().iterator();
		//Achievement test[] = new Achievement[20];
		List<Achievement> test = new ArrayList<Achievement>();
		selectedType = type;
		while (it.hasNext()) {
			Entry<String, Achievement> entry = it.next();
			Achievement achievement = entry.getValue();
		...
		int index2 = 1;
		for(Achievement achieve : test){
			if(completeAchievement(achieve))
				player.getPackets().sendIComponentText(3043, index2, "<col=" + ChatColors.GREEN + ">"+achieve.getTitle()+"");
			else
				player.getPackets().sendIComponentText(3043, index2, "<shad=><col=" + ChatColors.RED+ ">"+achieve.getTitle()+"");
			index2++;
		}
		
	}
	/**
	 * sends the info empty
	 */
	public void sendEmpty(){
		player.getPackets().sendIComponentText(3043, 29, "");
		player.getPackets().sendIComponentText(3043, 30, "");
		player.getPackets().sendIComponentText(3043, 31, "");
		for (int i =0; i <5; i++)
			player.getPackets().sendHideIComponent(3043,46+i,true);
	}
	/**
	...
	public void handelButtons(int buttonId){
		...
		if(buttonId >= 32 && buttonId <= 45){
			Iterator<Entry<String, Achievement>> it = getAchievements().entrySet().iterator();
			ArrayList<Achievement> test = new ArrayList<Achievement>();
			while (it.hasNext()) {
				Entry<String, Achievement> entry = it.next();
				Achievement achievement = entry.getValue();
			...
			sendInfo(test.get(buttonId - 32));
		}else if(buttonId == 55)
			player.closeInterfaces();
		else if(buttonId == 52)
			sendTestInterface(Types.MEDIUM);
		else if(buttonId == 51)
			sendTestInterface(Types.EASY);
		else if(buttonId == 53)
			sendTestInterface(Types.HARD);
		else if(buttonId == 54)
			sendTestInterface(Types.ELITE);
	}
	/**
	 * sends achievement info
	 * @param achieve
	 * 
	 */
	public void sendInfo(Achievement achieve){
		int total = achieve.getTotalAmount();
		int amt = achieve.getAmountFinished(player);
		player.getPackets().sendIComponentText(3043, 29, achieve.getTitle());
		player.getPackets().sendIComponentText(3043, 30, achieve.getDescription()); 
		player.getPackets().sendIComponentText(3043, 31, amt+"/"+total);
		int index = 46;
		for (int i =0; i <5; i++)
			player.getPackets().sendHideIComponent(3043,46+i,false);
		for(Item item : achieve.getRewards()){
			player.getPackets().sendItemOnIComponent(3043, index, item.getId(), item.getAmount());
			index++;
		} for(int xd = index; xd <= 50; xd++ ){
			player.getPackets().sendHideIComponent(3043,xd,true);
		}
		//
	}
	...
	public void displayInformation(Player player) {
		//Scrollable.sendQuestScroll(player, "Achievement Help", "<col=" + ChatColors.BLUE + ">Information:", "You have already started all the achievements, all you need to do is progress in them to complete them. Press \"easy achievements\" and check out some examples", "", "<col=" + ChatColors.BLACK + ">Achievement Points:", "These are points that are given by some achievements that can be exchanged in the achievement store for <col=" + ChatColors.RED + ">crazy</col> rewards!", "", "<col=" + ChatColors.BLACK + ">Other Rewards", "Some achievement give item rewards to you. If you are not doing a minigame, they will be given in your inventory as long as you have space for it. <col=" + ChatColors.PURPLE + ">If you don't have space, it is placed in your bank!");
	}
