...
public final class EmotesManager implements Serializable {
	...
	player.stopAll();
		ClueScrolls.completedRiddle(player, id);
	if (id == 2) {// Yes
		...
		Object[][] skillcapes = {
				...
				{"defence", 4961, 824,1},
				{"strength", 4981, 828,1},
				{"constitution",14242, 2745,1},
				{"ranged", "ranging", 4973, 832,1},
				{"magic", 4939, 813,1},
				{"prayer", 4979, 829,1},
				{"cooking", 4955, 821,278},
				{"woodcutting", 4957, 822,1},
				{"fletching", 4937, 812,1},
				{"fishing", 4951, 819,1},
				{"firemaking", 4975, 831,1},
				{"crafting", 4949, 818,1},
				{"smithing", 4943, 815,1},
				{"mining", 4941, 814,1},
				{"herblore", 4969, 835,1},
				...
				{"slayer", 4967, 1656,1},
				{"farming", 4963, 825,1},
				{"runecrafting", 4947, 817,1},
				{"construction", 4953, 820,1},
				{"summoning", 8525, 1515,1},
				{"hunter", 5188, 907,1},
				{"quest", 4945, 816,1},
			//	{"dungeoneering", 13190, 2442},
			//	{"classic", 122, Utils.getRandom(2) == 0 ? 1471 : 1466},
		};
		final String capeName = player.getEquipment().getItem(Equipment.SLOT_CAPE).getName().toLowerCase();
		for (int i = 0; i < skillcapes.length; i++) {
				if (capeName.split(" ")[0].equalsIgnoreCase(skillcapes[i][0].toString())) {
					Animation animation = new Animation(Integer.parseInt(skillcapes[i][1].toString()));
					Graphics graphics = new Graphics(Integer.parseInt(skillcapes[i][2].toString()));
					NPC npc1 = World.spawnNPC((int)skillcapes[i][3], new WorldTile(player.getX() + 1, player.getY(), player.getPlane()), -1, false);
					NPC npc2 = World.spawnNPC((int)skillcapes[i][3], new WorldTile(player.getX() - 1, player.getY(), player.getPlane()), -1, false);
					npc1.setRandomWalk(false);
					npc2.setRandomWalk(false);
					npc1.setCantInteract(true);
					npc2.setCantInteract(true);
					npc1.setDirection(player.getDirection());
					npc2.setDirection(player.getDirection());
					player.setNextAnimation(animation);
					player.setNextGraphics(graphics);
					npc1.setNextAnimation(animation);
					npc1.setNextGraphics(graphics);
					npc2.setNextAnimation(animation);
					npc2.setNextGraphics(graphics);
		...
		if (capeName.contains("dungeoneering")) {
		...
		} else if (capeName.contains("veteran")) {
			if (capeName.contains("10 year")) {
				if (!World.isTileFree(player.getPlane(), player.getX(), player.getY(), 3)) {
					player.getPackets().sendGameMessage("You need clear space outside in order to perform this emote.");
					return;
			...
			} else if (capeName.contains("5 year")) {
				if (!World.isTileFree(player.getPlane(), player.getX(), player.getY(), 3)) {
					player.getPackets().sendGameMessage("You need clear space outside in order to perform this emote.");
					return;
				} else if (player.getControlerManager().getControler() != null) {
			...
			} else if (capeName.contains("completionist")) {
				if (!World.isTileFree(player.getPlane(), player.getX(), player.getY(), 3)) {
					player.getPackets().sendGameMessage("You need clear space outside in order to perform this emote.");
					return;
				} else if (player.getControlerManager().getControler() != null) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						if (step == 0) {
							...
							player.getAppearence().transformIntoNPC(player.getEquipment().getCapeId() == 20769 ? 1830 : 3372);
		...
		player.setNextAnimation(new Animation(12449));
	} else if (id == 73) {// Begging
		player.setNextAnimation(new Animation(12450));
	} else if (id == 74) {// Stir Cauldron
		player.setNextAnimation(new Animation(12463));
	} else if (id == 75) {// Cheer
		player.setNextAnimation(new Animation(12473));
		...
		player.setNextAnimation(new Animation(12472));
	} else if (id == 79) {// Point
		player.setNextAnimation(new Animation(12476));
	} else if (id == 80) {// Punch
		player.setNextAnimation(new Animation(12477));
	} else if (id == 81) {// Raise Hand
		player.setNextAnimation(new Animation(12484));
	    ...
	    if (!World.isTileFree(player.getPlane(), player.getX(), player.getY(), 3)) {
		player.getPackets().sendGameMessage("You need clear space in order to perform this emote.");
		return;
	    } else if (player.getControlerManager().getControler() != null) {
		player.getPackets().sendGameMessage("You can't do this here.");
		return;
		...
		player.getPackets().sendGameMessage("There is not enough space around you to do this emote.");
		return;
	    }
