...
import com.rs.utils.Utils;
	...
	private enum Maps {
		Map1(337, 2971, 3414,
				...
				338, 3021, 3912,
				"In beetween a lava blaze and a near Deathly Agility Course!"), Map3(
				339, 2722, 3339,
	...
	public static void giveEliteReward(Player player) {
		Item[] rewards = null;
		if (Misc.random(10) == 1) {
			rewards = new Item[] {
					AnyRewards[Utils.getRandom(AnyRewards.length -1)],
					EliteCommonRewards[Utils.getRandom(EliteCommonRewards.length -1)],
					EliteCommonRewards[Utils.getRandom(EliteCommonRewards.length -1)],
					EliteRareRewards[Utils.getRandom(EliteRareRewards.length -1 )]};
		} else {
			rewards = new Item[] {
					AnyRewards[Utils.getRandom(AnyRewards.length -1)],
					EliteCommonRewards[Utils.getRandom(EliteCommonRewards.length-1)],
				...
				for (Item item : rewards) {
					if (item != null)
						if (player.getInventory().addItemMoneyPouch(item))
							//World.addGroundItem(item, new WorldTile(player), player, true, 180,true);
						player.getPackets().sendGameMessage("Congratulations, you have won "+Utils.formatAorAn(item)+" "
							+item.getName()+"!", true);
						player.getInterfaceManager().sendInterface(364);
						player.getPackets().sendItems(141, rewards);
						if (item.getName().contains("Third-age") 
								|| item.getName().contains("Third age") 
								|| item.getName().contains("briefcase") 
								|| item.getName().contains("suitcase")
								|| item.getName().contains("explosive")
								|| item.getName().contains("sack of effigies")
								|| item.getName().toLowerCase().contains("dye")
								|| item.getName().toLowerCase().contains("ardougne")) {
							World.sendWorldMessage("<img=6><col=ff8c38>News: "+player.getDisplayName()+" has just recieved "+Utils.formatAorAn(item)+" "
								+item.getName().toLowerCase()+" from a clue scroll.", false);
							rewards = null;
						}
	...
	public static void giveReward(Player p) {
		if (p.cluenoreward >= 1 && p.cluenoreward <= 5) {
			...
			p.clueChance += Utils.random(70, 100);
			else if (p.clueLevel == 2)
			p.clueChance += Utils.random(60, 80);
			else if (p.clueLevel == 3)
			p.clueChance += Utils.random(30,60);
		}
		if (p.clueChance > 150) {
			Item[] rewards = null;
			if (p.clueLevel == 0) {
			...
			p.getPackets().sendGameMessage("<col=1C9800>You have completed </col><col=ff0000>"+p.clueScrolls+"</col><col=1C9800> clue scroll"+(p.clueScrolls > 1 ? "s" : "")+".");
			p.cluenoreward = 0;
			for (Item item : rewards) {
					if (item != null)
						if (!p.getInventory().addItemMoneyPouch(item))
						//	World.addGroundItem(item, new WorldTile(p), p, true, 180,true);
						p.getPackets().sendGameMessage("Congratulations, you have won "+Utils.formatAorAn(item)+" "
							+item.getName()+"!", true);
						...
						if (item.getName().contains("Third-age") 
								...
								|| item.getName().toLowerCase().contains("dye")
								|| item.getName().toLowerCase().contains("ardougne")) {
							World.sendWorldMessage("<img=6><col=ff8c38>News: "+p.getDisplayName()+" has just recieved "+Utils.formatAorAn(item)+" "
								+item.getName().toLowerCase()+" from a clue scroll.", false);
