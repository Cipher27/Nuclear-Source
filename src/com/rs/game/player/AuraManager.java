...
public class AuraManager implements Serializable {
	...
	public void activate() {
		...
		if(toId != -1) {
			...
			/*if (!player.isLegendaryDonator() && isWingedAura(item.getId())) {
				player.getPackets().sendGameMessage("You need to be a legendary donator to use this feature.");
				return;
			}*/
			int tier = getTier(item.getId());
			/*if (!player.isLegendaryDonator() && tier > 1) {
				player.getPackets().sendGameMessage("You need to be a legendary donator to use this feature.");
				return;
			}*/
 ...
 */
	...
	public double getAttackAccuracyMultiplier() {
		if (!isActivated() || World.isPvpArea(player) || player.isCanPvp())
			return 1;
		int aura = player.getEquipment().getAuraId();
		if (aura == 22897) //berserker
			return 1.10;
		return 1;
	}
	public double getStrengthAccuracyMultiplier() {
		if (!isActivated() || World.isPvpArea(player) || player.isCanPvp())
			return 1;
		int aura = player.getEquipment().getAuraId();
		if (aura == 22897) //berserker
	...
	public double getDefenceAccuracyMultiplier() {
		if (!isActivated() || World.isPvpArea(player) || player.isCanPvp())
			return 1;
		int aura = player.getEquipment().getAuraId();
		if (aura == 22897) //berserker
