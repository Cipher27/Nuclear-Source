 ...
 *
 */
public class DonationTokenManager extends Dialogue {
	...
	public void run(int interfaceId, int componentId) {
		if(stage == -1){
			...
			stage = 1;
				...
				sendOptions("Select an option", 
						"Set the donatorzone as spawn place "+(player.donatorRespawn ? Colors.green+"Enabled" : Colors.red+"Disabled")+"", 
						"Donator icon "+(player.showIcon ? Colors.green+"Enabled" : Colors.red+"Disabled")+"",
						"Yell color : <col="+player.getYellColor()+">click to change");
				stage = 10;
			} else {
				sendOptions("Select an option", "Donator grenwal area", "Celestial dragons");
				stage = 5;
			}
		} else if (stage == 1) {
			if(componentId == OPTION_1){
		 ...
		 */
			...
			if(componentId == OPTION_1){
				if(player.donatorRespawn){
					player.donatorRespawn = false;
					sendOptions("Select an option", 
							"Set the donatorzone as spawn place "+(player.donatorRespawn ? Colors.green+"Enabled" : Colors.red+"Disabled")+"", 
							"Donator icon "+(player.showIcon ? Colors.green+"Enabled" : Colors.red+"Disabled")+"",
							"Yell color : <col="+player.getYellColor()+">click to change");
		 ...
		 */
			...
			sendNPCDialogue(npcId, 9827, "You need atleast one free inventory space to claim this item.");
			stage = 21;
		}
		} else if(stage == 21){
