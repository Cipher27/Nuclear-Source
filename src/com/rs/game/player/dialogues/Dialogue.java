...
public abstract class Dialogue {
	...
	public static int JUST_LISTEN = 9804,
			CALM_TALKING = 9808, CALM = 9808, LOOK_DOWN = 9812;
	public static int WHAT_THE = 9816,ANGRY = 9788, WHAT_THE_TWO = 9820, 
			CROOKED_HEAD = 9828;
	...
	public static String[][] gender = { {"mr", "mrs"}, {"mister", "miss"}, {"lord", "lady"}, {"boy", "girl"}, {"man", "woman"}, {"sir", "m'am"} };
	...
	public boolean sendNpc(int npcId,String... text) {
		return sendEntityDialogue(IS_NPC, npcId, 9827, text);
	}
	...
	public boolean sendPlayer(int animationId, String... text) {
		return sendEntityDialogue(IS_PLAYER, -1, animationId, text);
	}
	
	public static boolean sendPlayerDialogue2(int animationId,Player player, String... text) {
		return sendTestDialogue(IS_PLAYER, "", -1,animationId, player, text);
		//return sendEntityDialogue(IS_PLAYER, -1, animationId,player, text);
	}
	...
	public boolean sendOptions(String title, String... options) {
		return sendOptionsDialogue(title,options);
	}
	...
	public boolean sendDialogue2(short interId, String... talkDefinitons) {
	 ...
	 public boolean sendEntityDialogue2(short interId, String[] talkDefinitons,
		...
		if(type == IS_NPC) {
			if (!player.getAppearence().isMale()) {
				for (int i = 0; i < gender.length; i++) {
					if (text.contains(gender[i][0]))
						text = text.replace(gender[i][0], gender[i][1]);
				}
			}
	...
	public static boolean sendTestDialogue(int type, String title, int entityId, int animationId,Player player, String... texts) {
		StringBuilder builder = new StringBuilder();
		for(int line = 0; line < texts.length; line++) 
				builder.append(" " + texts[line]);
		String text = builder.toString();
