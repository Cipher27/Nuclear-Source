...
public final class LegendaryPetLevelUp extends Dialogue {
	...
	public void start() {
		...
		int level = player.getSkills().getLegendaryPetLevelForXp(skill);
		...
		String name = Skills.SKILL_NAME_LegendaryPet[skill];
		...
		player.getPackets().sendGameMessage(
				"Your pet just advanced a" + (name.startsWith("A") ? "n" : "")
	...
	public static void sendNews(Player player, int skill, int level) {
			...
			World.sendWorldMessage(
					"<img=6><col=ff8c38>News: " + player.getDisplayName()
							+ " has achieved " + level + " "
							+ Skills.SKILL_NAME_LegendaryPet[skill] + ".", false);
			}
			return;
}
