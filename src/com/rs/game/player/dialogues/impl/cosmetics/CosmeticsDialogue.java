...
public class CosmeticsDialogue extends Dialogue {
	...
	public void start() {
		...
		if (slotId == Equipment.SLOT_ARROWS) {
			...
			int[] costumes = slotId == Equipment.SLOT_HAT ? CosmeticsHandler.HATS : slotId == Equipment.SLOT_CAPE ? CosmeticsHandler.CAPES : slotId == Equipment.SLOT_AMULET ? CosmeticsHandler.AMULETS : slotId == Equipment.SLOT_CHEST ? CosmeticsHandler.CHESTS : slotId == Equipment.SLOT_LEGS ? CosmeticsHandler.LEGS : slotId == Equipment.SLOT_HANDS ? CosmeticsHandler.GLOVES : slotId == Equipment.SLOT_WEAPON ? CosmeticsHandler.WEAPONS : slotId == Equipment.SLOT_SHIELD ? CosmeticsHandler.SHIELDS : slotId == Equipment.SLOT_AURA ? CosmeticsHandler.WINGS : CosmeticsHandler.BOOTS;
			for (int costumeItemId : costumes) {
				if (costumeItemId == -1)
					continue;
				if (keyWord != null && !CosmeticsHandler.getNameOnDialogue(costumeItemId).toLowerCase().contains(keyWord))
					continue;
				availableCostumes.add(costumeItemId);
			}
		}
		if (keyWord != null && availableCostumes.isEmpty()) {
			player.getDialogueManager().startDialogue("SimpleMessge", "We couldn't find any available costume with keyword:" + keyWord + " .");
			return;
		}
		currentPage = 0;
		if (player.getEquipment().getCosmeticItems().get(slotId) != null) {
			sendOptionsDialogue(DEFAULT_OPTIONS_TITLE, (player.getEquipment().getHiddenSlots()[slotId] ? "Unhide" : "Hide"), "Remove current costume", "Nevermind");
		} else
	...
	private String[] getDialogueOptions() {
		...
		if (slotId == Equipment.SLOT_RING) {
			...
			ArrayList<Cosmetic> savedCosmetics = player.getEquipment().getSavedCosmetics();
			dialogueOptions.add(currentPage == 0 ? (Utils.formatPlayerNameForDisplay(savedCosmetics.get(pages[currentPage][0]).getCosmeticName())) : "Back");
			...
			maxPagesNeeded = ((int) Math.ceil(availableCostumes.size() / 3.00));
			maxPagesNeeded = maxPagesNeeded == 0 ? 1 : maxPagesNeeded;
			int index = 0;
			...
			dialogueOptions.add(currentPage == 0 ? (slotId == Equipment.SLOT_ARROWS ? (player.getEquipment().isHideAll() ? "Unhide All" : "Hide All") : (player.getEquipment().getHiddenSlots()[slotId] ? "Unhide" : "Hide")) : "Back");
			int itemsCount = getItemsCount();
			if (slotId == Equipment.SLOT_ARROWS) {
				for (int i = 0; i < itemsCount; i++) {
					boolean restricted = CosmeticsHandler.isRestrictedOutfit(player, pages[currentPage][i]);
					dialogueOptions.add((restricted ? "<col=ff0000>" : "<col=00ff00>") + CosmeticsHandler.OUTFIT_NAMES[pages[currentPage][i]]);
				}
			} else {
				for (int i = 0; i < itemsCount; i++) {
	...
	public void run(int interfaceId, int componentId) {
		...
		case 0:
			...
			case OPTION_2:
				...
				player.getPackets().sendGameMessage("You have removed " + choosenCosmetic.getCosmeticName() + " from your saved cosmetics.");
				end();
				break;
			case OPTION_3:
				stage = -1;
	...
	public void sendSavedCosmeticOptions() {
		stage = 0;
		sendOptionsDialogue(DEFAULT_OPTIONS_TITLE, "Choose this outfit.", "Remove this outfit.", "Back.", "Cancel.");
	}
	...
	public void setCostume(Item item) {
		...
		if ((slotId == Equipment.SLOT_ARROWS && item.getDefinitions().getEquipSlot() == Equipment.SLOT_WEAPON) || slotId == Equipment.SLOT_WEAPON) {
		...
		int[] costumes = itemSlotId == Equipment.SLOT_HAT ? CosmeticsHandler.HATS : itemSlotId == Equipment.SLOT_CAPE ? CosmeticsHandler.CAPES : itemSlotId == Equipment.SLOT_AMULET ? CosmeticsHandler.AMULETS : itemSlotId == Equipment.SLOT_CHEST ? CosmeticsHandler.CHESTS : itemSlotId == Equipment.SLOT_LEGS ? CosmeticsHandler.LEGS : itemSlotId == Equipment.SLOT_HANDS ? CosmeticsHandler.GLOVES : itemSlotId == Equipment.SLOT_WEAPON ? CosmeticsHandler.WEAPONS : itemSlotId == Equipment.SLOT_SHIELD ? CosmeticsHandler.SHIELDS : itemSlotId == Equipment.SLOT_AURA ? CosmeticsHandler.WINGS : CosmeticsHandler.BOOTS;
		...
		if (!hasItem) {
			player.getPackets().sendGameMessage("<col=ff0000>We couldn't find " + item.getName() + " in your available outfits.");
			return;
		}
		player.getEquipment().getCosmeticItems().set(slotId == Equipment.SLOT_ARROWS || slotId == Equipment.SLOT_RING ? itemSlotId : slotId, item);
		player.getAppearence().generateAppearenceData();
	}
