...
public class TanningD extends Dialogue {
	...
	player.getActionManager().setAction(new Action() {
		...
		int maximumQuantity = price == 0 ? requestedAmount : player.getInventory().getAmountOf(995) / price;
			maximumQuantity += price == 0 ? requestedAmount : player.getMoneyPouch().getCoinsAmount() / price;
		...
		if (player.getInventory().getAmountOf(995) < price && player.getMoneyPouch().getCoinsAmount() < price) {
		    end();
		...
		if (price != 0){
			if(player.getMoneyPouch().getCoinsAmount() >= price){
