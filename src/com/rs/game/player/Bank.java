...
public class Bank implements Serializable {
	...
	public void depositAllInventory(boolean banking) {
		boolean full = false;
		for(int i = 0; i < 28; i++) {
			if (MAX_BANK_SIZE - getBankSize() <= 0) {
				final Item item = player.getInventory().getItem(i);
				if (item != null && !containsItem(item.getId(), 1)) {
					full = true;
	...
	/*public void depositMoneyPouch(boolean banking) {
		if (player.getMoneyPouch().getCoinsAmount() <= 0) {
			player.getPackets().sendGameMessage("Your money pouch is empty!");
			return;
		}
		int coinsCount = player.getMoneyPouch().getCoinsAmount() > Integer.MAX_VALUE ? Integer.MAX_VALUE : player.getMoneyPouch().getCoinsAmount();
		...
		if (space != 0) {
			...
			player.getMoneyPouch().sendAddOrRemove(player.getMoneyPouch().getCoinsAmount(), true);
		}
	}*/
	public void depositMoneyPouch(boolean banking) {
		int depositable = player.getMoneyPouch().getCoinsAmount();
		if (depositable <= 0) {
			player.getPackets().sendGameMessage("Your money pouch is empty!");
			return;
		}
		final Item coins = player.getBank().getItem(995);
		if (coins != null) {
			final int amount = coins.getAmount();
			if (depositable + amount <= 0)//fixes integer overflow
				depositable = (Integer.MAX_VALUE - amount);
			if (depositable <= 0) {
				player.getPackets().sendGameMessage("Not enough space in your bank.");
				return;
			}
		}
		int space = addItems(new Item[] { new Item(995, depositable) }, banking);
		if (space != 0) {
			//if (Bank.MAX_BANK_SIZE - getBankSize() < player.getInventory().getItems().getSize()) {
			if (MAX_BANK_SIZE - getBankSize() < 1 && coins == null) {
				player.getPackets().sendGameMessage("Not enough space in your bank.");
				return;
			}
			player.getMoneyPouch().sendAddOrRemove(depositable, true);
		}
	}
	...
	public void switchItem(int fromSlot, int toSlot) {
		int[] fromRealSlot = getRealSlot(fromSlot);
		if(insertItems) {
			//insertItem(fromSlot, fromRealSlot[0], toSlot);//old nully method
	...
	/*public void insertItem(int fromSlot, int tab, int index) {//old nully method
		if(index == fromSlot)
			return;
	...
	public void openDepositBox() {
		/*if (player.isUltimate()) {
			player.sm("Ultimate ironman accounts do not have access to this.");
			return;
		}*/
	...
	public void openBank() {
		...
		player.getVarsManager().sendVar(638, -1);
		player.getVarsManager().sendVar(638, 0);
		//CLEM
	...
	public void addItem(int id, int quantity, int creationTab, boolean refresh) {
		if (ItemDefinitions.getItemDefinitions(id).isNoted())
			id = ItemDefinitions.getItemDefinitions(id).certId;//automatically notes items when being added to the bank, where applicable
...
/*	public boolean removeItem(int fakeSlot, int quantity, boolean refresh, boolean forceDestroy) {
	...
	public int[] getItemSlot(int id) {
		for(int tab = 0; tab < bankTabs.length; tab++) {
			for(int slot = 0; slot < bankTabs[tab].length; slot++) {
				if (bankTabs[tab][slot] != null && bankTabs[tab] != null)
	...
	public int getItemForKey(String keyword) {
		for(int tab = 0; tab < bankTabs.length; tab++) {
			for(int slot = 0; slot < bankTabs[tab].length; slot++) {
				if(ItemDefinitions.getItemDefinitions(bankTabs[tab][slot].getId()).getName().toLowerCase().contains(keyword.toLowerCase()))
					return bankTabs[tab][slot].getId();
			}
		}
		return 0;
	}

	public boolean removeItemC(int[] slot, int quantity, boolean refresh, boolean forceDestroy) {
        if(slot == null)
            return false;
        Item item = bankTabs[slot[0]][slot[1]];
        boolean destroyed = false;
