...
public class Range extends Default {
	...
	public int[] getSpecialIds() {
		return new int[] { 11235, 11212 };
	}

	public int[] getWeapons() {
		return new int[] { 11283, 11284, 11285, 1201, 3842, 9185, 9244 };
	}
	...
	public void processCombat() {
		...
		if (!robot.getCombatDefinitions().isUsingSpecialAttack()) {
			if (robot.getCombatDefinitions().getSpecialAttackPercentage() >= PlayerCombat
					.getSpecialAmmount(getSpecialIds()[0])) {
				if (getAttackedBy() != null
						&& getAttackedBy().getHitpoints() <= 400) {
					if (!wearingSpecialWeapon()
							&& robot.getInventory().hasFreeSlots()) {
						for (int i = 0; i < getSpecialIds().length; i++)
							ButtonHandler.sendWear(robot, robot.getInventory()
									.getItems().lookupSlot(getSpecialIds()[i]),
									getSpecialIds()[i]);
	...
	public boolean wearingSpecialWeapon() {
		return (robot.getEquipment().getWeaponId() == getSpecialIds()[0]);
	}
}
