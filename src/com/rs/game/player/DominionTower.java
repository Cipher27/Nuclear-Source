...
public final class DominionTower implements Serializable {
	...
	private static final Item[] REWARD_CHEST = new Item[] { new Item(3024, 1),
			new Item(6685, 1), new Item(2448, 1), new Item(15328, 1),
			new Item(565, 10), new Item(566, 10), new Item(560, 10),
			new Item(561, 10), new Item(892, 10), new Item(9144, 10),
			new Item(532, 1), new Item(536, 1), new Item(532, 1),
			new Item(6729, 1) };
	
	private static final Item[] RARE_REWARDS = new Item[] {new Item(22346, 1), new Item(22347, 1),
		new Item(22348, 1), new Item(22358, 1), new Item(22362, 1),
		new Item(22366, 1), new Item(22370, 1)};
	...
	/**
	 * handles the buttons of all the interfaces
	 * @param interfaceId
	 * @param componentId
	 * @param packetId
	 * @param slotId
	 */
	public void handleButtons(int interfaceId, int componentId, int packetId,
			int slotId) {
		if (interfaceId == 1164) {
			if (componentId == 26)
				openClimberMode();
	...
	private static final Boss[] BOSSES = { 
			...
			new Boss("Jad", "Adam shall feast upon your bones!", 2745),
	...
	/**
	 * opens the reward  after the you die/leave
	 */
	public void openRewardsChest() {
		...
		rewards = new ItemsContainer<Item>(dominionFactor > 100000 ? 20
				: dominionFactor / 5000 + 1, false);
		for (int i = 0; i < rewards.getSize(); i++) {
			Item item;
			if (Misc.random(10) == 1)
				item = (RARE_REWARDS[Utils.random(RARE_REWARDS.length)]);	
			else
				item = (REWARD_CHEST[Utils.random(REWARD_CHEST.length)]);
			if (item.getId() == 22358 || item.getId() == 22362
					|| item.getId() == 22366)
		...
		player.getPackets().sendInterSetItemsOptionsScript(1171, 7, 100, 8, 3,
				"Take", "Bank", "Discard", "Examine");
		player.getPackets().sendUnlockIComponentOptionSlots(1171, 7, 0, 20, 0,
				1, 2, 3);
		refreshRewards();
	}
