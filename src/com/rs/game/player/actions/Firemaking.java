...
public class Firemaking extends Action {
	...
	MAGIC(1513, 75, 900, 70765, 270, 1), 
	CURSED_MAGIC(13567, 82, 1000, 70766, 303.8, 1),
	ELDER(29556,90,1000, 70766, 303.8, 1);
	...
	if (!quickFire){
		if(player.getInventory().contains(32637) || player.getEquipment().getWeaponId() == 32637){
			  player.setNextAnimation(new Animation(25183));
		} else 
	...
	if (!usingPyre) {
	    if (!player.getInventory().containsItemToolBelt(590) && !player.getInventory().containsItem(32637,1) && (player.getEquipment().getWeaponId() != 32637)) {
	...
	if (player.getEquipment().getRingId() == 13659)
	    totalXp *= 1.025;
	if (player.getEquipment().getWeaponId() == 32637 || player.getInventory().containsItem(32637,1))
	    totalXp *= 1.025;
	return totalXp;
    }
	...
	if (player.getDailyTask() != null)
		player.getDailyTask().incrementTask(player, 3, fire.getLogId(), Skills.FIREMAKING);
