...
public class Cooking extends Action {
	...
	public boolean start(Player player) {
		...
		if (cook.isFireOnly() && !object.getDefinitions().name.equals("Fire")) {
			...
			player.sm("You need a cooking level of " + cook.getLvl() + " to cook this food.");
	...
	private boolean isBurned(Cookables cook, Player player) {
		...
		if(player.getPerkHandler().perks.contains(Perk.COOKING_PERK))
			return false;
	...
	public int processWithDelay(Player player) {
		amount--;
		if (player.SamuraiCooking == true && player.ArcaneCooking == false) {
			player.setNextAnimation(new Animation(17314));
			player.setNextGraphics(new Graphics(3306));
		}
		if (player.SamuraiCooking == false && player.ArcaneCooking == true) {
			player.setNextAnimation(new Animation(20298));
			player.setNextGraphics(new Graphics(3999));
		} else if (player.SamuraiCooking == false && player.ArcaneCooking == false) {
		...
		if ((player.getSkills().getLevel(Skills.COOKING) >= cook.getBurningLvl()) ? false : isBurned(cook, player)) {
			...
			player.getPackets().sendGameMessage("You accidently burnt the " + cook.getProduct().getDefinitions().getName().toLowerCase() + ".", true);
		} else {
			player.getInventory().deleteItem(item.getId(), 1);
			player.getInventory().addItem(cook.getProduct().getId(), cook.getProduct().getAmount());
			player.randomevent(player);
			if (object.getId() == 89768) {
				player.getSkills().addXp(Skills.COOKING, cook.getXp() + cook.getXp() / 100 * 21);
				if (Utils.random(20) == 1)
					player.sm("This cooking range gave you an extra" + cook.getProduct().getDefinitions().getName().toLowerCase() + " and banked it for you.");
				player.getBank().addItem(cook.getProduct(), true);
			} else {
			...
			player.getUrns().addMaterial(SkillingUrns.CRACKED_COOKING, cook.getXp(), Skills.COOKING);
			if (player.getDailyTask() != null)
				player.getDailyTask().incrementTask(player, 3, item.getId(), Skills.COOKING);
		}
		if (amount > 0) {
			player.getPackets().sendGameMessage("You attempt to cook the " + cook.getProduct().getDefinitions().getName().toLowerCase() + ".");
			return 3;
