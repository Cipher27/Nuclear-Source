	...
	public static boolean clean(final Player player, Item item, final int slotId) {
		...
		if (player.getSkills().getLevel(Skills.HERBLORE) < herb.getLevel()) {
			player.getPackets().sendGameMessage(
					"You do not have the required level to clean this, you need "+herb.getLevel()+" herblore for this action.", true);
			return true;
		}
		if(player.getPerkHandler().perks.contains(Perk.HERBLORE_PERK)){
		int amount = player.getInventory().getAmountOf(herb.herbId);
		player.getSkills().addXp(Skills.HERBLORE, herb.getExperience() * amount);
		player.getInventory().deleteItem(new Item(herb.herbId,amount));
		player.getInventory().addItem(new Item(herb.cleanId,amount));
		player.getInventory().refresh();
		player.getPackets()
				.sendGameMessage("You cleaned the herbs.", true);
		} else {
