...
public class Summoning {
	...
	public static void spawnFamiliar(Player player, Pouches pouch) {
		if (player.getFamiliarDelay() > Utils.currentTimeMillis()) {
			player.getPackets()
					.sendGameMessage("You have to wait 5 seconds before spawning a familiar after dissmissing.");
			return;
		}
		...
		try {
			return (Familiar) Class
					.forName("com.rs.game.npc.familiar."
							+ (NPCDefinitions.getNPCDefinitions(getNPCId(pouch.getRealPouchId(),player))).getName()
	...
	public static enum Pouches { 
		...
		private Pouches(int pouchSetting, int realPouchId, double minorExperience, double experience, long pouchTime,
				int summoningCost) {
	...
	public static int getNPCId(int id, Player player) {
		int original = ClientScriptMap.getMap(1320).getIntValue(id);
		player.originalId = original;
		if(player.getNpcOverrideId() != 0 )//original == 6873)
			return player.getNpcOverrideId();
		return original;
	}
	...
	public static void handlePouchInfusion(Player player, int itemId, int amount) {
		
		try {
			Item item = new Item(itemId);
			SummoningPouch pouch = SummoningPouch.get(itemId);
			if (pouch == null) {
				player.sm("This pouch is currently unavailable. Please try back at a later time.");
				return;
			}
			if (player.getSkills().getLevelForXp(Skills.SUMMONING) < pouch.getLevelRequired()) {
				player.sm("You don't have the level required to create that pouch.");
				return;
			}
			for (Item req : pouch.getItems()) {
				if (!player.getInventory().contains(req)) {
					String name = req.getDefinitions().getName();
					int a = req.getAmount();
					String s = !name.endsWith("s") && a > 1 ? "s" : "";
					player.sm("You still need "+a+" "+name+""+s+".");
				}
			}
			if (!player.getInventory().contains(pouch.getItems())) {
				//message for items is above
				return;
			}
			...
			for (Item i : pouch.getItems()) {
				int amountCanMake = (int) Math.floor(player.getInventory().getNumerOf(i.getId()) / i.getAmount());
				if (amountCanMake < amount)
					amount = amountCanMake;
				/*if (!player.getInventory().containsItem(i.getId(), i.getAmount() * amount)) {
					player.sendMessage("You can't create that many!");
					return;
				}*/
			}
			...
			player.infusedPouches += amount;
			double experience = pouch.getCreateExperience() * amount;
			player.getSkills().addXp(Skills.SUMMONING, experience);
			player.getPackets().sendObjectAnimation(new WorldObject(28716, 10, 1, 2209, 5344, 0), new Animation(8509));
			player.sm("You infuse " + (amount == 1 ? "a" : "some") + " " + item.getName().toLowerCase() + (amount == 1 ? "" : "es") + ".");
			//if (Misc.random(1000) == 1) 
				//player.getInventory().addItem(32512, 1);
