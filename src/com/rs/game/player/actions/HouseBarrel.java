...
public class HouseBarrel extends Action {
	...
	public static boolean handleItemOnObject(Player player, Item itemUsed,
		...
		if (itemUsed.getId() == BEER_GLASS && object.getId() >= BEER_BARREL && object.getId() <= CHEF_BARREL) {
			...
			player.getActionManager().setAction(new HouseBarrel(player.getInventory().getNumerOf(BEER_GLASS)));
				return true;
			}
		return false;
	}
	...
	private boolean checkAll(Player player) {
		if (player.getInventory().getNumerOf(BEER_GLASS) < 1) {
			player.sm("You need an empty glass to do this.");
			return false;
		}
		return true;
	}
