		...
		int roll = Misc.random(100);
		 Player owner = tree.getOwner();
			if(player == owner && roll >= 60) {
				...
				player.getPackets().sendGameMessage("You cut a " + logName + ".", true);			
			} else {
				String logName = ItemDefinitions.getItemDefinitions(definitions.getLogsId()).getName().toLowerCase();
				if(owner != null){
				owner.getInventory().addItem(notedlog, 1);
				owner.sendMessage(""+player.getUsername()+" cut some "+logName+" for you.");	
				}
				player.getInventory().addItem(log, 1);
				player.getPackets().sendGameMessage("You cut some " + logName + ".", true);
			}
