...
public class DivineHunting extends Action {
	...
	public enum DivineHuntingSpots {
		...
		DIVINE_KEBBIT_BURROW(87270, 1, -1, new Animation(21236), 10,  Herbs.polar_kebbit_fur, Herbs.raw_beast_meat, Herbs.common_kebbit_fur, Herbs.feldip_weasel_fur, Herbs.desert_devil_fur, Herbs.long_kebbit_spike),
		
		DIVINE_BIRD_SNARE(87271, 1, -1, new Animation(21236), 10,  Herbs.red_feathers, Herbs.raw_bird_meat, Herbs.yellow_feathers, Herbs.orange_feathers, Herbs.blue_feathers, Herbs.stripy_feathers, Herbs.wimpy_feathers),
		
		DIVINE_DEADFALL_TRAP(87272, 1, -1, new Animation(21236), 10, Herbs.kebbit_claws, Herbs.barb_tail_harpoon, Herbs.kebbit_spike, Herbs.diseased_kebbit_fur, Herbs.kebbit_teeth),
		
		DIVINE_BOX_TRAP(87273, 1, -1, new Animation(21236), 10, Herbs.Chinchompas, Herbs.red_chinchompas, Herbs.raw_pawya_meat, Herbs.grenwall_spikes);
	...
	public boolean start(Player player) {
		...
		player.getPackets().sendGameMessage("You start hunting from the divine.",  true);
	...
	private int getFishingDelay(Player player) {
		...
		if (wcTimer < 1 + spot.getRandomLifeProbability())
			wcTimer = 1 + Utils.getRandom(spot.getRandomLifeProbability());
	...
	public int processWithDelay(Player player) {
		addFish(player);
	 if (Utils.getRandom(spot.getRandomLifeProbability()) == 0) {
		player.stopAll();
	...
	private void addFish(Player player) {
		Item fish = new Item(spot.getHerbs()[fishId].getId(), multipleCatch ? 2
				: 1);
		Item notedfish = new Item(spot.getHerbs()[fishId].getId()+1, multipleCatch ? 2
				: 1);
		...
		int roll = Misc.random(100);
		 Player owner = spott.getOwner();
			if(player == owner && roll >= 60) {
				...
				player.getPackets().sendGameMessage("You caught a " + fishName + ".", true);			
			} else {
				String fishName = fish.getDefinitions().getName().toLowerCase();
				if(owner != null){
				owner.getInventory().addItem(notedfish);
				owner.sendMessage(""+player.getUsername()+" caught some "+fishName+" for you.");	
				}player.getInventory().addItem(fish);
				player.getPackets().sendGameMessage("You caught some " + fishName + ".", true);
				}
	...
	if (multipleCatch)
			return "Your high level in hunter allows you to catch two " + fish.getDefinitions().getName().toLowerCase() + ".";
		else
			return "You manage to catch a " + fish.getDefinitions().getName().toLowerCase() + ".";
	}

	private boolean checkAll(Player player) {
		if (player.getSkills().getLevel(Skills.HERBLORE) < spot.getHerbs()[fishId].getLevel()) {
			player.getDialogueManager().startDialogue("SimpleMessage","You need a hunter level of " + spot.getHerbs()[fishId].getLevel() + " to hunt here.");
			return false;
		}
		if (!player.getInventory().containsOneItem(spot.getBait()) && spot.getBait() != -1) {
			player.getPackets().sendGameMessage("You don't have " + new Item(spot.getBait()).getDefinitions().getName().toLowerCase() + " to hunt here.");
			return false;
		}
		...
		if(!World.containsObjectWithId(tile, spot.getId())) {
			player.getDialogueManager().startDialogue("SimpleMessage","The divine has disapeared.");
			player.setNextAnimation(new Animation(-1));
			return false;
