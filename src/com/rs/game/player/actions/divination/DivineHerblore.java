...
public class DivineHerblore extends Action {
	...
	public enum DivineHerbsSpots {
		...
		DIVINE_HERB_I(87280, 1, -1, new Animation(21237), 10,  Herbs.GUAM, Herbs.MARRENTILL, Herbs.TARRONMIN, Herbs.HARRALANDER, Herbs.RANARR, Herbs.SPIRIT_WEED, Herbs.TOADFLAX),
		
		DIVINE_HERB_II(87281, 1, -1, new Animation(21237), 10,  Herbs.IRIT, Herbs.WERGALI, Herbs.AVANTOE, Herbs.KWUARM, Herbs.SNAPDRAGON),
		
		DIVINE_HERB_III(87282, 1, -1, new Animation(21237), 10, Herbs.CADANTINE, Herbs.LANTADYME, Herbs.DWARF_WEED, Herbs.TORSTOL, Herbs.FELLSTALK);
	...
	public DivineHerblore(DivineHerbsSpots spot, int x, int y,int plane, WorldObject spott) {
		this.spot = spot;
		this.spott = spott;
	...
	public boolean start(Player player) {
		...
		player.getPackets().sendGameMessage("You start harvesting from the divine.",  true);
	...
	private int getRandomFish(Player player) {
		...
		int difference = player.getSkills().getLevel(Skills.FARMING)
	...
	private void addFish(Player player) {
		...
		int roll = Misc.random(100);
		 Player owner = spott.getOwner();
			if(player == owner && roll >= 60) {
				...
				player.getPackets().sendGameMessage("You harvest a " + fishName + ".", true);			
			} else {
				String fishName = fish.getDefinitions().getName().toLowerCase();
				
				if(owner!= null){
					owner.getInventory().addItem(notedfish);
					owner.sendMessage(""+player.getUsername()+" harvest some "+fishName+" for you.");	
					}
				player.getInventory().addItem(fish);
				player.getPackets().sendGameMessage("You harvest some " + fishName + ".", true);
				}
		...
		if (player.getFamiliar() != null) {
			if (Utils.getRandom(50) == 0 && getSpecialFamiliarBonus(player.getFamiliar().getId()) > 0) {
				player.getInventory().addItem(fish);
				player.getSkills().addXp(Skills.FARMING, 5.5);
			}
		}
		fishId = getRandomFish(player);
	}
	...
	if (multipleCatch)
			return "Your high level in farming allows you to harvest two " + fish.getDefinitions().getName().toLowerCase() + ".";
		else
			return "You manage to harvest a " + fish.getDefinitions().getName().toLowerCase() + ".";
	}

	private boolean checkAll(Player player) {
		if (player.getSkills().getLevel(Skills.FARMING) < spot.getHerbs()[fishId].getLevel()) {
			player.getDialogueManager().startDialogue("SimpleMessage","You need a farming level of " + spot.getHerbs()[fishId].getLevel() + " to harvest here.");
			return false;
		}
		if (!player.getInventory().containsOneItem(spot.getBait()) && spot.getBait() != -1) {
			player.getPackets().sendGameMessage("You don't have " + new Item(spot.getBait()).getDefinitions().getName().toLowerCase() + " to harvest here.");
			return false;
		}
