...
public class PrifddinasPickpocketing extends Action {
	...
	public boolean process(Player player) {
		...
		if (checkAll(player)) {
			...
			if (Utils.random(player.getSkills().getLevel(Skills.THIEVING) * 10) == 0) {
				...
				player.lock(3);
				if (player.getCaught >= 3) {
				player.sm("The thieves have caught you and kicked you out of their castle.");
                player.getCaught = 0;		
                player.setNextWorldTile(new WorldTile(3288,3916,0));				
				}
			}
			return true;
		}
	...
	public int processWithDelay(Player player) {
		...
		addItems(player);
		player.masterThieves++;
		return Utils.random(20, 50);
	}
