...
public class RobustGlassMachine extends Action {
	...
	public static final Item SANDSTONE = new Item(23194, 1);
	public static final Item ROBUST_GLASS = new Item(23193, 1);
	private int quantity;
	private MachineData data;

	public static final int INGREDIENTS[] = { 23194 };
	...
	public enum MachineData {
		ROBUST_GLASS(23194, 1, 23193, 150)
		;
		...
		private static Map<Integer, MachineData> MachineItems = new HashMap<Integer, MachineData>();

		public static MachineData forId(int id) {
			return MachineItems.get(id);
		}

		static {
			for (MachineData machine : MachineData.values()) {
				MachineItems.put(machine.finalProduct, machine);
			}
		}

		private MachineData(int ingredientId, int ingredientAmount, int finalProduct, double experience) {
	...
	public static boolean handleItemOnObject(Player player, Item item,
		...
		for (int i = 0; i < INGREDIENTS.length; i++) {
			if (item.getId() == INGREDIENTS[i]
				...
				player.getDialogueManager().startDialogue("RobustGlassMachineD",
						MachineData.forId(PRODUCTS[index][0]));
				return true;
				}
			}
		}
	...
	public RobustGlassMachine(MachineData data, int quantity) {
		this.data = data;
