...
public class Bonfire extends Action {
	...
	public int processWithDelay(Player player) {
		if (player.getDailyTask() != null)
			player.getDailyTask().incrementTask(player, 3, log.logId, Skills.FIREMAKING);
		if(player.getEquipment().getCapeId() == 31289 && Utils.random(1000) <= 4){
			int amount = player.getInventory().getAmountOf(log.logId);
			player.getInventory().deleteItem(log.logId, amount);
			player.getSkills().addXp(Skills.FIREMAKING, Firemaking.increasedExperience(player, log.xp) * amount);
			player.sm("Your mastercape burned all your remaining logs.");
		}
		...
		if (player.PartyhatFiremaking == true) {
		player.setNextAnimation(new Animation(16699));
		player.setNextGraphics(new Graphics(Utils.random(4722, 4735)));	
		} else {
