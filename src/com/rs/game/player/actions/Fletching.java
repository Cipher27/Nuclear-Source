...
public class Fletching extends Action {
	...
	public static enum Fletch {
		...
		ELDER_BOW(29556, 946, new int[] { 29736, 29734, 30372}, new int[] { 90, 95, 97 },
				new double[] { 101.2, 116.4, 130.25}, new Animation(6702)),
		...
		PROTEAN_LOGS(679, 946, new int[] { -1 }, new int[] { 1},
				new double[] { 91.5 }, new Animation(26267)),

		//int id, int selected, int[] product, int level[],double[] xp, Animation anim
		/**
		 * Crossbows
		 */
		...
		DRAGONBANE_BOLTS(21843, 314, new int[] { 21660 }, new int[] { 70 },
				new double[] { 7 }, new Animation(6702)),

		DRAGON_BOLTS(9193, 9144, new int[] { 9341 }, new int[] { 71 },
				new double[] { 8.2 }, new Animation(6702)),
			

		ONYX_BOLTS(9194, 9144, new int[] { 9342 }, new int[] { 73 },
				new double[] { 9.4 }, new Animation(6702)),
		
		BLOODWOOD(24121, 946, new int[] { 24122 }, new int[] { 93 },
				new double[] { 13 }, new Animation(24942)),
		
		BAKRIMINEL_BOLTS(24127, 24122, new int[] { 24116 }, new int[] { 93 },
				new double[] { 13 }, new Animation(-1)),
		
		/*ASCENSION_BOLTS(28436, 946, new int[] { 28465 }, new int[] { 90 },
				new double[] { 20 }, new Animation(-1)),*/
				
		ASCENDRI_BOLTS(31867, 28465, new int[] {31868}, new int[] {80},
				new double[] {10.6}, new Animation(-1)),
				
		/**
		 * slayer ammo
		 */
		BROAD_ARROWS(13278, 53, new int[] { 4160 }, new int[] { 52 },
				new double[] {14.2 }, new Animation(-1)),
						
		BROAD_BOLTS(13279, 314, new int[] { 13280 }, new int[] { 55 },
				new double[] { 9.1 }, new Animation(-1)),
		...
		PEARL_BOLT_TIPS(411, CHISLE, new int[] { 46 }, new int[] { 41 }, new double[] { 2.5 }, new Animation(886)),

		TOPAZ_BOLT_TIPS(1613, CHISLE, new int[] { 9188 }, new int[] { 48 }, new double[] { 5 }, new Animation(887)),

		SAPPHIRE_BOLT_TIPS(1607, CHISLE, new int[] { 9189 }, new int[] { 56 }, new double[] { 7 }, new Animation(888)),

		EMERALD_BOLT_TIPS(1605, CHISLE, new int[] { 9190 }, new int[] { 58 }, new double[] { 11 }, new Animation(889)),

		RUBY_BOLT_TIPS(1603, CHISLE, new int[] { 9191 }, new int[] { 63 }, new double[] { 17 }, new Animation(887)),

		DIAMOND_BOLT_TIPS(1601, CHISLE, new int[] { 9192 }, new int[] { 65 }, new double[] { 24 }, new Animation(890)),

		DRAGON_BOLT_TIPS(1615, CHISLE, new int[] { 9193 }, new int[] { 71 }, new double[] { 33 }, new Animation(885)),

		ONYX_BOLT_TIPS(6573, CHISLE, new int[] { 9194 }, new int[] { 73 }, new double[] { 44 }, new Animation(2717)),
		//dream staff
		DRAMEN_BRANCH(771, KNIFE, new int[] { 772 }, new int[] { 1 }, new double[] { 50 }, new Animation(6702)),
        //Jadinko items.
		SAGIE_SHAFTS(21351, KNIFE, new int[] { 21353 }, new int[] { 83 }, new double[] { 64 }, new Animation(3007)),

		SAGIE(21358, 21353, new int[] { 21364 }, new int[] { 83 }, new double[] { 68 }, new Animation(3113)),

		BOLAS(21359, 21358, new int[] { 21365 }, new int[] { 87 }, new double[] { 73 }, new Animation(3113));
	...
	//	int amount = maxMakeQuantityTen(fletch) ? 15 : 1;
		...
		int amount = stackable1 ? 15 : 1;
		int amount2 = ItemDefinitions.getItemDefinitions(fletch.getId()).isStackable() ? 15 : 1;
		int amount3 = ItemDefinitions.getItemDefinitions(fletch == Fletch.SAGIE_SHAFTS ||fletch == Fletch.SAGIE_SHAFTS ? 5 : fletch.getProduct()[option]).isStackable() ? 15 : 1;
		if (stackable1) {
			int amt1 = player.getInventory().getAmountOf(fletch.getSelected());
			int amt2 = player.getInventory().getAmountOf(fletch.getId());
			if (amt1 < amount) {
		...
		if (player.getDailyTask() != null && ItemDefinitions.getItemDefinitions(fletch.getId()).getName().toLowerCase().contains("(u)"))
				//handles stringing unstrung bows
				player.getDailyTask().incrementTask(player, 3, fletch.getId(), Skills.FLETCHING);
		if (player.getDailyTask() != null 
					&& (ItemDefinitions.getItemDefinitions(fletch.getProduct()[option]).getName().toLowerCase().contains("(u)") || 
							ItemDefinitions.getItemDefinitions(fletch.getProduct()[option]).getName().toLowerCase().contains("bolas")))
						//handles producting unfinished bows and bolas
						player.getDailyTask().incrementTask(player, 3, fletch.getProduct()[option], Skills.FLETCHING);
		...
		if(fletch.getProduct()[option] == 30372)
			player.getInventory().addItem(fletch.getProduct()[option], 3);
			else
