...
public class OwnedObjectManager {
	...
	public static boolean replaceObject(final Player player, WorldObject object,
			final WorldObject newObject, final long time) {
		for (Iterator<String> it = player.getOwnedObjectManagerKeys()
				.iterator(); it.hasNext();) {
			final OwnedObjectManager manager = ownedObjects.get(it.next());
			if (manager == null) {
			...
			if (manager.getCurrentObject().getX() == object.getX()
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						manager.delete();
						addOwnedObjectManager(player,
								new WorldObject[] { new WorldObject(newObject) },time );
					}
				});
				return true;
			}
