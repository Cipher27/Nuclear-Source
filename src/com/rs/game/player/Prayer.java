...
public class Prayer implements Serializable {
	...
	if (npc.getBonuses() != null) {
	    leechBonuses[8] = (int) ((100 * Math
		    .floor(0.15 * npc.getBonuses()[0])) / npc.getBonuses()[0]);
	    leechBonuses[9] = (int) ((100 * Math
		    .floor(0.15 * npc.getBonuses()[9])) / npc.getBonuses()[9]);
	    leechBonuses[10] = (int) ((100 * Math
		    .floor(0.1 * npc.getBonuses()[6])) / npc.getBonuses()[6]);
	    adjustStat(0, leechBonuses[8]);
	    adjustStat(1, leechBonuses[10]);
	    adjustStat(2, leechBonuses[9]);
	} else {
	...
	private boolean checkPrayer() {
		if (prayerpoints <= 0) {
			...
			player.getPackets().sendGameMessage(
					"Please recharge your prayer at home.");
			return false;
		}
		return true;
	}
