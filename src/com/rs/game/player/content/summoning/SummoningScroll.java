...
public class SummoningScroll {
	...
	/**
	 * An enum containing all Scroll data.
	 * @author Vichy
	 */
    ...
    public static void createScroll(Player player, int itemId, int amount) {
		...
		if (!player.getInventory().containsItem(scroll.pouchId, 1)) {
		    player.sendMessage("You do not have enough " + 
		    		ItemDefinitions.getItemDefinitions(scroll.pouchId).getName().toLowerCase()+ "es for this action.");
		    return;
		}
		if (player.getSkills().getLevelForXp(Skills.SUMMONING) < scroll.reqLevel) {
		    player.sendMessage("You need a summoning level of " + scroll.reqLevel + " to create "
				    + amount + " "  + ItemDefinitions.getItemDefinitions(scroll.scrollId).getName().toLowerCase() + "s.");
		    return;
		}
		...
		player.sendMessage("You have created some "+ 
				ItemDefinitions.getItemDefinitions(scroll.scrollId).getName().toLowerCase() + "s.");
