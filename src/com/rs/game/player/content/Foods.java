...
public class Foods {
		...
		CABAGE_MESSAGE {
			...
			public void effect(Object object) {
				...
				player.getPackets().sendGameMessage(
						"You love it. Because everyone loves cabbage", true);
			}
		},
	...
	public static final Animation EAT_ANIM = new Animation(18001);
	...
	public static boolean eat(final Player player, Item item, int slot) {  
		...
		long foodDelay = name.contains("half") ? 600 : 1200;
		player.getActionManager().setActionDelay((int) foodDelay / 600);
	...
	//	player.getActionManager().setActionDelay(player.getActionManager().getActionDelay() + 2);
		...
		double amount = (player.getPerkHandler().perks.contains(Perk.HEAL_PERK) ? ((food.getHeal() * 10)*0.15 + food.getHeal() * 10) : (food.getHeal() * 10));
		player.heal((int)amount, food.getExtraHP() * 10);
