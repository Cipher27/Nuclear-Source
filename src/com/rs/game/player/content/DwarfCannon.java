...
public class DwarfCannon implements Serializable {
	...
	public void checkLocation() {
		...
		} else if (DwarfCannon.isRestrictedCannon(player)){
	...
	public void cannonSetup() {
		if(!player.completedDwarfCannonQuest){
			player.sm("You need to complete the mini dwarven quest first.");
			return;
		}
		if (hasCannon()) {
	...
	public void cannonSetupRoyal() {
		...
		if(!player.getArtisanWorkshop().royal_Cannon){
			player.sm("You have not unlocked this feature, you can unlock it in the Artisan workshop.");
			return;
		}
		if (!getPlayer().getInventory().containsItems(
	...
	public void cannonSetupGold() {
		...
		if(!player.getArtisanWorkshop().golden_Cannon){
			player.sm("You have not unlocked this feature, you can unlock it in the Artisan workshop.");
			return;
		}
		if (!getPlayer().getInventory().containsItems(
	...
	public boolean fireDwarfCannon(WorldObject object) {
		boolean hit = false;
		if (getCannonBalls() == 0) {
			if(player.getArtisanWorkshop().restocking){
				if(player.getInventory().contains(2)){
					int amount = player.getInventory().getAmountOf(2) > 90 ? 90 : player.getInventory().getAmountOf(2);
