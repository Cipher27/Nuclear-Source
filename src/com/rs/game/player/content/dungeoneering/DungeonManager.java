...
public class DungeonManager {
	...
	public void openRoom(final Room room, final RoomReference reference, final VisibleRoom visibleRoom) {
		...
		RegionBuilder.copy2RatioSquare(room.getChunkX(party.getComplexity()), room.getChunkY(party.getFloorType()), toChunkX, toChunkY, room.getRotation(), 0, 1);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				for (Player player : party.getTeam()) {
					...
					player.sm("Please note that dungeoneering is stil under construction, there are bugs. Paolo will fix these asap, enjoy ur dungeon!");
	...
	public void wearInventory(Player player) {
		...
		for (int slotId = 0; slotId < 28; slotId++) {
			...
			if (item == null)
				continue;
			/*if (ButtonHandler.wear(player, item.getId(), slotId))
				worn = true;*/
		}
		if (worn) {
			player.getAppearence().generateAppearenceData();
	...
	public DungeonNPC spawnNPC(int id, int rotation, WorldTile tile, RoomReference reference, int type, double multiplier) {
		DungeonNPC n = null;
	...
	public int[] getBonuses(boolean boss, int level) {
		...
		bonuses[CombatDefinitions.RANGE_ATTACK] = party.getRangeLevel() + (level / 10);
		bonuses[CombatDefinitions.STAB_ATTACK] = (int) (party.getAttackLevel() + (level / 10) / 1.2);
		bonuses[CombatDefinitions.MAGIC_ATTACK] = (int) (party.getAttackLevel() + (level / 10) / 1.5);
		int npcDefenceLevel = /*party.getDefenceLevel()*/party.getAverageCombatLevel() / (boss ? 5 : 10);
		//bonuses[CombatDefinitions.RANGE_DEF] = npcDefenceLevel * + (level / (boss ? 2 : 3));
		bonuses[CombatDefinitions.STAB_DEF] = bonuses[CombatDefinitions.CRUSH_DEF] = bonuses[CombatDefinitions.SLASH_DEF] = (int) ((npcDefenceLevel + (level / (boss ? 10 : 30))) / 1.2); // 20%

		//usualy range def > melee def for mobs but bows in dung suck, and even range gear doesnt help that much
		bonuses[CombatDefinitions.RANGE_DEF] = (int) ((npcDefenceLevel + (level / (boss ? 10 : 30))) / 1.6); // 60% nerf

		// less
		// than
		// range
		bonuses[CombatDefinitions.MAGIC_DEF] = (int) ((npcDefenceLevel + (level / (boss ? 10 : 30))) / 1.5); // 50%
		// less
		// than
		// range
		if (Settings.DEBUG)
	...
	public void destroy() {
		synchronized (this) {
			...
			CoresManager.slowExecutor.schedule(new Runnable() {
				...
				public void run() {
					try {
						RegionBuilder.destroyMap(oldboundChuncks[0], oldboundChuncks[1], oldDungeon.getMapWidth() * 2, oldDungeon.getMapHeight() * 2);
					} catch (Throwable e) {
						Logger.handle(e);
					}
				}
	...
	public void loadRewards() {
		...
		for (Player player : party.getTeam()) {
			if (party.getSize() == DungeonConstants.TEST_DUNGEON) {
				player.getPackets().sendGameMessage("Moving next floor.");
				player.getSkills().addXp(Skills.DUNGEONEERING, 500000);
				player.getDungManager().setTokens(player.getDungManager().getTokens() + 50000);
			...
			for (int i = 0; i < 5; i++) {
				...
				player.getPackets().sendGlobalConfig(1198 + i, partyPlayer != null ? 1 : 0); //sets true that this player exists
				if (partyPlayer == null)
					continue;
				//player.getPackets().sendGlobalString(2383 + i, partyPlayer.getDisplayName());
			...
			player.getPackets().sendGlobalConfig(1188, party.getSize() + 1); //dungeon size, sets bonus aswell
			multiplier += DungeonConstants.DUNGEON_SIZE_BONUS[party.getSize()];
			player.getPackets().sendGlobalConfig(1191, party.getTeam().size() * 10 + party.getDificulty()); //teamsize:dificulty
			...
			player.getPackets().sendGlobalConfig(1195, v); //dungeons rooms opened, sets bonus aswell
			...
			player.getPackets().sendGlobalConfig(1196, party.isGuideMode() ? 1 : 0); //sets guidemode
			if (party.isGuideMode())
				multiplier -= 0.05;
		...
		//	if (party.getTeam().size() == 1 && party.getSize() != DungeonConstants.LARGE_DUNGEON)
			...
			player.getSkills().addXp(Skills.DUNGEONEERING, totalXp); //force rs xp, cuz we do * xp rate in calcs to make inter show correct xp
			player.getPointsManager().setDungeoneeringTokens(player.getPointsManager().getDungeoneeringTokens() + tokens);
			player.getMusicsManager().forcePlayMusic(770);
			if (!tickedOff) {
