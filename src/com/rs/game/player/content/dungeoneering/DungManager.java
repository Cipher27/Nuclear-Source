...
public class DungManager implements Serializable {
	...
	public void openPartyInterface() {
		player.getInterfaceManager().sendTab(player.getInterfaceManager().hasRezizableScreen() ? 114 : 174, 939);
	...
	public void refreshPartyDetailsComponents() {
		if (!player.getInterfaceManager().containsInterface(939))
			return;
		player.getPackets().sendHideIComponent(939, 37, party != null);// Form
																		// party
																		// button
		player.getPackets().sendHideIComponent(939, 31, party == null || !party.isLeader(player));// Leave
																									// Group
		player.getPackets().sendHideIComponent(939, 38, party == null || !party.isLeader(player));// Invite
																									// Button
		player.getPackets().sendHideIComponent(939, 105, party != null && party.isLeader(player));// Complexity
																									// change
		player.getPackets().sendHideIComponent(939, 111, party != null && party.isLeader(player));// Floor
																									// change
		player.getPackets().sendHideIComponent(939, 95, party != null && party.isLeader(player));// Guide
																									// mode
		...
		for (int component = 29; component < 31; component++) {
			if (party == null)
				player.getPackets().sendHideIComponent(939, component, true);
			else
				player.getPackets().sendHideIComponent(939, component, (party.isLeader(player)) ? false : true);
		}

		/*
		 * for (int i = 0; i < 5; i++) { Player p2 = party == null || i >=
		 * party.getTeam().size() ? null : party.getTeam().get(i); for (int i2 =
		 * 35 + i * 3; i2 < 38 + i * 3; i2++)
		 * player.getPackets().sendHideIComponent(939, i2, p2 == null); }
		 */
	...
	private void hideSkills(int complexity) {
		...
		for (int i = 0; i < COMPLEXITY_SKILLS.length; i++) {
			player.getPackets().sendIComponentText(938, 90 + i, (i >= count ? "<col=383838>" : "") + COMPLEXITY_SKILLS[i]);
		}
	}
	...
	public void enterDungeon(boolean selectSize) {
		...
		for (Player p2 : party.getTeam()) {
			...
			if (p2.getPlane() != 0 || p2.getInterfaceManager().containsScreenInter() || p2.isLocked() || !(p2.getControlerManager().getControler() instanceof Kalaboss)) {
