...
public static final int[] SOF_JACKPOT_OTHERS = new int[] { 995, 995, 995, 995, 20929, 23671, 23672, 25202 };
  ...
  */
	...
	if (interfaceId == 1139) { // squeal tab
		...
		openSpinInterface();
	    	...
	    	player.getDialogueManager().startDialogue(new Dialogue() {
				...
				@Override
				public void start() {
					sendOptions("Quick spin options", "1x","5x", "10x", "50x","100x");
					stage = 1;
     ...
     */
    private void quickSpin(int amount){
	...
	if (!discard) {
	    if (type == SOF_STATUS_CLAIMPOUCHOK) {
	    	...
	    	/*Masks mask = Masks.hasMask(rewards[rewardSlot].getId());
	 	    if(mask != null){
	 	    	player.getSofItems2().add(rewards[rewardSlot].getId());
	 	    }¨*/
	 	//cosmetic shit
	...
	if (isLamp) {
	   ...
	   /* Masks mask = Masks.hasMask(itemId);
	    if(mask != null){
	    	if(player.getSofItems2().contains(itemId)){
	    		while(player.getSofItems2().contains(itemId) || mask != null){
	    			itemId = items[Utils.random(items.length)];
	    			mask = Masks.hasMask(itemId);
	    		}
	    	}
	    }*/
	    /**
	     * check on the cosmetic items
	     */
	    for(int i = 0; i < cosmetics.length; i ++){
