...
import com.rs.game.player.achievements.impl.SoManyRunesAchievement;
...
public final class Runecrafting {
	...
	public static void craftEssence(Player player, int rune, int level,
		...
		if (hasRcingSuit(player)||hasFullBloodEtherealEquipped(player))
			totalXp *= 1.025;
		...
		player.lock(5);
		if (rune == 565 && hasFullBloodEtherealEquipped(player)){
			player.getInventory().addItem(rune, runes*2);	
			player.sm("Your set doubled the amount of blood runes you crafted");
		} else 
			player.getInventory().addItem(rune, runes);
		if(rune == 565){
			player.bloodRunesCrafted += runes;
			player.getAchievementManager().notifyUpdate(SoManyRunesAchievement.class);
		}
	...
	public static boolean hasFullBloodEtherealEquipped(Player player){
		if(player.getEquipment().getHatId() == 32347 && player.getEquipment().getChestId() == 32348
				&& player.getEquipment().getLegsId() == 32349 && player.getEquipment().getBootsId() == 32351
