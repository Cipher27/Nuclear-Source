...
public class PrayerBooks {
	...
	{ 3839, 3841, 3843, 19612, 19614, 19616 };
	private static final int[] ANIMATIONS =
	{ 1335, 1336, 1337, -1, -1, -1 };
	private static final int[] PAGE_BASE =
	{ 3827, 3831, 3835, 19600, 19604, 19608 };
	...
	private static final String[][] CHANTS =
	{
...

		{
			...
			"The weak deserve to die, so the strong may flourish. This is the will of Zamorak.",
			...
			"Battle is my calling, and death shall be my rest. Zamorak give me strength!",
...

		{
			...
			"Big High War God say: You not run away from battle, or Big High War God strike you.",
			"Big High War God say: Good warrior kill enemies, before he get squished.",
			"Big High War God say: Follow and obey your commander, unless they run away.",
			"Big High War God say: If you not worthy of Big High War God, you get made dead soon.",
			"Big High War God say: If your friend get killed, be happy; they were too weak for Big High War God.",
			"Big High War God say: War is best, Peace is for weak.", },
			...
			"To those cursed by war and pest, Come into the light of Armadyl and rest. This is the law of Armadyl.", },
		{
			...
			"Though your enemies wish to silence thee, Do not falter, defy them to the end. Power to the Great Lord!",
			"The followers of the Great Lord are few, But they are powerful and mighty. Power to the Great Lord!",
			"Follower of the Great Lord be relieved: One day your loyalty will be rewarded. Power to the Great Lord!",
			"Pray for the day that the Great Lord rises; It is that day thou shalt be rewarded. Power to the Great Lord!",
			"Oppressed thou art, but fear not: The day will come when the Great Lord rises. Power to the Great Lord!",
			"Fighting oppression is the wisest way, To prove your worth to the Great Lord. Power to the Great Lord!", } };
	...
	public static void bindPages(Player player, int bookId) {
		...
		for (int pageIndex = 0; pageIndex < 4; pageIndex++) {
			if (!player.getInventory().containsItem(PAGE_BASE[god] + pageIndex, 1)) {
				player.sm("You need 4 different pages to get the god book");
		...
		if (containsGodPages) {
			...
			player.getPackets().sendGameMessage("You bind all four pages into the book.");
			player.getPrayerBook()[god] = true;
		}

	}

	public static void sermanize(Player player, final int bookId) {
		final int god = (bookId - (bookId > 5000 ? BOOKS[3] - 6 : BOOKS[0])) / 2;
		player.getDialogueManager().startDialogue(new Dialogue() {

			@Override
			public void start() {
				sendOptionsDialogue("Select a relevant passage.", "Wedding Cerimoney", "Last Rites", "Blessings", "Preach");
			}

			@Override
			public void run(int interfaceId, int componentId) {
				int ordinal = getOrdinal(componentId);
				String[] passages = CHANTS[god];
				String message = passages[ordinal];
