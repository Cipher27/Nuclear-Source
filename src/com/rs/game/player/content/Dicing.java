...
public class Dicing {
	...
	public static void privateRoll(final Player player, final int itemId,
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				player.sendDiceRoll(player.getUsername()+" rolled <col=db3535>"
								+ getRandom(lowest, highest) + "</col> on "
								+ diceText(itemId) + " die.");
			}
		}, 1);
	...
	public static void friendsRoll(final Player player, final int itemId,
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				player.getInventory().addItem(itemId, 1);
                World.sendWorldMessage("<col=00E633><shad=000000> "+ player.getUsername() +" rolled <col=FF0000>" + getRandom(lowest, highest) +"</col> " + "on the percentile dice", false);
                player.setNextForceTalk(new ForceTalk("You rolled <col=FF0000>" + getRandom(lowest, highest) +"</col> " + "on the percentile dice"));
