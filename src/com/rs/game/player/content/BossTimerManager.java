	...
	public void recieveDeath(NPC npc) {
		...
		if (isBoss(npc)) {
			...
			if (timeTaken < player.FastestTime[currentIndexValue] || player.FastestTime[currentIndexValue] == 0) {
				...
				player.sendMessage("New personal Best for " + npc.getName() + ".");
				player.sendMessage("Your new time to beat is " + Utils.formatTime(timeTaken) + ".");
				player.sendMessage("Your old time was " + Utils.formatTime(oldTime)+ ".");
			} else {
				player.sendMessage("You killed " + npc.getName() + " in " +Utils.formatTime(timeTaken) + " seconds.");
			}
			if (timeTaken < World.FastestTime[currentIndexValue] || World.FastestTime[currentIndexValue] == 0) {
				if (World.playerLeader[currentIndexValue] == null) {
					World.sendWorldMessage(player.getUsername() + " has set a new server record for killing " + npc.getName() + " his time was " + Utils.formatTime(timeTaken), false);
				} else {
					World.sendWorldMessage(player.getUsername() + " has set a new server record for killing " + npc.getName() + " his time was " + Utils.formatTime(timeTaken) + ". They beat " + World.playerLeader[currentIndexValue] + "'s time of " + Utils.formatTime(World.FastestTime[currentIndexValue]), false);
				}
