...
public class Assassins implements Serializable {
    ...
    public void completeTask() {
    	...
    	//player.getSkills().addLegendaryPetXp(getGameMode(), addExp());
    	if (taskAmount <= 0) {
    		player.sm("You have successfully completed your task and have receieved an LegendaryPet's Favor Token.");
    	//	player.getSkills().addLegendaryPetXp(0, addExp()*5);
    ...
    public void activateSwiftSpeed() {
    	...
    	final int time = player.getSkills().getLegendaryPetLevel(Skills.SWIFT_SPEED)*30;
    	WorldTasksManager.schedule(new WorldTask() {
    		int loop = time;
    		@Override
    		public void run() {
    ...
    public void activateCallLegendaryPet() {
    	player.sm("You have successfully called an LegendaryPet.");
