...
public class Wilderness extends Controler {
	...
	public boolean keepCombating(Entity target) {
		...
		if (player.getCombatDefinitions().getSpellId() <= 0 && Utils.inCircle(new WorldTile(3105, 3933, 0), target.getWorldTile(), 24)) {
	...
	/**
	 * handles the magic teleports in the wilderness
	 */
	@Override
	public boolean processMagicTeleport(WorldTile toTile) {
		if(Warbands.isCarryingResources(player)){ //can't teleport with warbands item
			player.sm("An item in your inventory prevents you from teleporting.");
			return false;
		}
		if (getWildLevel() > 20) { //wilderness levle needs to be lower than 20
			player.getPackets().sendGameMessage("A mysterious force prevents you from teleporting.");
			return false;
		}
		if (player.getTeleBlockDelay() > Utils.currentTimeMillis()) { //teleblock
			player.getPackets().sendGameMessage("A mysterious force prevents you from teleporting.");
			return false;
		}
		if(player.getInventory().containsItem(24474,1) ||player.getEquipment().containsOneItem(24474)){ //wildy bow
			player.getDialogueManager().startDialogue("SimpleMessage", "You can't teleport with this bow, you have to destroy it first.");
			return false;
		}
		return true;
	...
	public boolean processItemTeleport(WorldTile toTile) {
		...
		if (getPlayer().getTeleBlockDelay() > Utils.currentTimeMillis()) {
	 ...
	 * 
	 * @param id object id
	 * @return true if id is the id of the wilderness ditch
	 */
	...
	public void moved() {
		...
		if (!showingSkull && isAtWild && !isAtWildSafe) {
			...
			removeIcon();
			removeControler();
		} else if (Kalaboss.isAtKalaboss(player.getTile())) {
			removeIcon();
			player.setCanPvp(false);
			removeControler();
     ...
     * 
     * @param tile
     * @return true if player is at wild
     */
	...
	public static int getWildLevel(Player player) {
		if (player.getY() > 9900)
			return 5 + ((player.getY() - 9912) / 8 + 1);
		return 5 + ((player.getY() - 3520) / 8 + 1);
	}
