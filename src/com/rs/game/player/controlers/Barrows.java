...
import com.rs.game.player.achievements.impl.TheLooterAchievement;
...
public final class Barrows extends Controler {
	...
	Item[] BARROW_REWARDS = { new Item(4708, 1), new Item(4710, 1), new Item(4712, 1), new Item(4714, 1),
			...
			new Item(4751, 1), new Item(4753, 1), new Item(4755, 1), new Item(4757, 1),new Item(4759), new Item(25652, 1),
			new Item(25672, 1) };
	...
	public void drop(Item item) {
		/**
		  * drop counting , todo make less laggy
		  **/
		 for (int itemId : Settings.RARE_DROP_IDS) {
				if (item.getId() == itemId) {
					int i = 1;
					if (player.getdropCount().containsKey(item.getId())) {
						i = player.getdropCount().get(item.getId()) + 1;
					}
					player.getdropCount().put(item.getId(), i);
				}
			}
		GlobalItemCounter.handleDropCount(item);
		Item dropItem = new Item(item.getId(), Utils.random(item.getDefinitions().isStackable()
				? item.getAmount()  : item.getAmount()) + 1);
		if (dropItem.getId() == 995) {
			player.getInventory().addItem(dropItem.getId(),dropItem.getAmount());
			return;
		}
	...
	public boolean processObjectClick1(WorldObject object) {
		if (object.getId() >= 6702 && object.getId() <= 6707) {
			...
			player.resetBarrows();
			player.barrowsChests++;
			player.getAchievementManager().notifyUpdate(TheLooterAchievement.class);
			player.sendMessage("You open the chest and find some treasures. You have looted : " + Colors.red+ Utils.getFormattedNumber(player.barrowsChests) + "</col>.");
			return false;
	...
	public void sendReward() {
		...
		drop(new Item(995, Utils.random(50000, 1000000))); // money reward at
															// least always
	}
