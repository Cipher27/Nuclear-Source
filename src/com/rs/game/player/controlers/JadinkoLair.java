...
public class JadinkoLair extends Controler { 
	...
	private static void createJadeRoot(int id, WorldTile tile, boolean permenant) {
		...
		if (permenant) {
			...
			World.spawnTemporaryObject(o, Utils.random(10000, 30000));
		}
	}
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					...
					if (ticks == 2) {
						...
						World.spawnTemporaryObject(new WorldObject(object.getId() + 1, object.getType(), object.getRotation(), object), 2000);
					} else if (ticks == 3) {
						player.setNextAnimation(new Animation(16700));
					} else if (ticks == 4) {
						World.spawnTemporaryObject(new WorldObject(object.getId() + 2, object.getType(), object.getRotation(), object), 30000);
	...
	public void magicTeleported(int teleType) {
		playersCount--;
		player.getInterfaceManager().sendOverlay(715,false);
		player.getControlerManager().forceStop();
	}
