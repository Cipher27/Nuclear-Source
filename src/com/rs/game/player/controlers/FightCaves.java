...
public class FightCaves extends Controler {
	...
	public static void enterFightCaves(Player player) {
		...
		if(player.isDonator())
			player.getControlerManager().startControler("FightCavesControler", 30);
		else
			...
			if(type == 1 || type == 4) {
				...
				if(type == 4) {
					...
					if(!player.getInventory().addItem(6570, 1)) {
						if(HallOfFame.firstFightCaves[0] == null){
							HallOfFame.firstFightCaves[0] = player.getDisplayName();
							HallOfFame.firstFightCaves[1] = java.time.LocalDate.now().toString();
							HallOfFame.save();
							player.isInHOF = true;
							player.sm("Congratz you are now in the hall of fame.");
						}
						WorldConstants.addFireCape();
