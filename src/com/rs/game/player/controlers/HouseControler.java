...
public class HouseControler extends Controler {
	...
	/**
	 * object actions for the first click
	 */
	@Override
	public boolean processObjectClick1(WorldObject object) {
		/**
		 * build is now first option
		 */
		...
		/* pet house */
		else if (HouseConstants.Builds.PET_HOUSE.containsObject(object)) {
			if (house.isOwner(player)) {
				house.getPetHouse().open();
			} else {
				player.getPackets().sendGameMessage("This isn't your pet house.");
			}
		}
		/*shelves */
	...
	public boolean handleItemOnObject(WorldObject object, Item item) {
		if (object.getId() == HouseConstants.HObject.CLAY_FIREPLACE.getId() || object.getId() == HouseConstants.HObject.STONE_FIREPLACE.getId() || object.getId() == HouseConstants.HObject.MARBLE_FIREPLACE.getId()) {
			...
			WorldObject objectR = new WorldObject(object);
			objectR.setId(object.getId() + 1);
			World.spawnObjectTemporary(objectR, 60000); //lights for 1min
			return false;
	...
	public boolean processObjectClick3(final WorldObject object) {
		if (Settings.DEBUG)
			System.out.println(object.getXInChunk() + ", " + object.getYInChunk() + ", 3");
	...
	public boolean processObjectClick4(WorldObject object) {
		if (Settings.DEBUG)
			System.out.println(object.getXInChunk() + ", " + object.getYInChunk() + ", 4");
	...
	public boolean login() {
		...
		removeControler();
		return false; //remove controller manualy since i don"t want to call forceclose
	}
