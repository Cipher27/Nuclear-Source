...
public class RunespanControler extends Controler {
	...
	public static WorldTile BONE_LADDER = new WorldTile(4297, 6040, 1);
	public static WorldTile BONE_LADDER_DOWN = new WorldTile(4107, 6042, 1);
	...
	private boolean handleCrossingPlataform(final WorldObject object, final Platforms plataform) {
		...
		if(largeIsland) 
			...
			for(int runeId : plataform.runes) {
				...
				if(runeId == ELEMENTAL_RUNE && (!fullEtherealEquipped(player))) {
				...
				}else if(!player.getInventory().containsOneItem(runeId) && (!fullEtherealEquipped(player))) {
			...
			for(int runeId : runes)  {
				...
				if (!fullEtherealEquipped(player))
			...
			player.getPackets().sendGameMessage(builder.toString()+" have been removed from your inventory.");
				...
				player.sm("Your set grand you free travels around runespan");
			}
	...
	public void start() {
		...
		player.getDialogueManager()
				.startDialogue(
						...
						"WARNING: When you leave all the runes you make here will be removed");
	...
	public void magicTeleported(int teleType) {
		for (int i = 24213; i < 24228; i++) {
			player.getInventory().deleteItem(i, 200000);
		}
		exitRunespan();
	...
	public void exitRunespan() {
		...
		player.getInterfaceManager().sendOverlay(1274, true);
   ...
   /**
    * sends the interfaces
    * i've hidden a comp because I have no use for it.
    */
	@Override
	public void sendInterfaces() {
		player.getInterfaceManager().sendOverlay(1274, false);
		player.getPackets().sendHideIComponent(1274, 1, true);
		player.getPackets().sendIComponentText(1274, 5, ""+player.getPointsManager().runespanStore);
		player.getPackets().sendIComponentText(1274, 2, "");
		refreshInventoryPoints();
	}
	private static final double[] REAL_POINTS_ORDER =
		{ 0.5, 0.3, 0.1, 0.4, 0.2, 0.7, 2.5, 1.5, 1.1, 1.7, 0.9, 1.3, 3, 3.5, 0, 0 };
	...
	public boolean processNPCClick1(NPC npc) {
		...
		case 15402:
			if (player.getInventory().containsItem(24227, 25)) {
				player.getPackets()
						.sendGameMessage(
								"You have already obtained enough essense from the floating essence. Get more from an essence creature.");
				return false;
			}
	...
	public boolean processObjectClick1(final WorldObject object) {
		...
		if(object.getId() == 70508) {
			...
			player.useStairs(16668, BONE_LADDER, 2, 3);
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
					player.setNextAnimation(new Animation(-1));
			...
			player.getPackets().sendGlobalConfig(1917, 3);
			return false;
		} else if (object.getId() == 70511) {
			player.useStairs(16675, BONE_LADDER_DOWN, 2, 3);
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
					player.setNextAnimation(new Animation(-1));
	...
	public static final boolean fullEtherealEquipped(Player player) {
		...
		if (helmId == -1 || chestId == -1 || legsId == -1||glovesId == -1 || bootsId == -1)
			return false;
