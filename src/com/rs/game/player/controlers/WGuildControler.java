...
public class WGuildControler extends Controler {
	...
	public void process() {
		...
		if(player.inClopsRoom()) {
			if(player.getWGuildTokens() <= 10) {
				player.sendMessage("Your time is about to run out. Please leave the cyclops room.");
			}
			if(player.getWGuildTokens() == 0) {
			...
			if(tick == 60) {
				...
				sendInterfaces();
				player.getPackets().sendGameMessage("You have lost 10 tokens while being in the cyclops room.", true);
				tick = 0;
			}
			tick++;
		}
	...
	public void enterClopsRoom() {
		if(player.getWGuildTokens() < 100) {
			player.getPackets().sendGameMessage("You must have more than 100 tokens to enter.");
			return;
		}
