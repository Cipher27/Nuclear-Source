...
import com.rs.game.player.dialogues.item.LeatherCraftingD;
...
public class DungeonControler extends Controler {
	...
	private void showDeaths() {
		player.getInterfaceManager().sendOverlay(945, false);
	}
	...
	public boolean sendDeath() {
		player.lock(8);
		player.stopAll();
		player.getPackets().sendMusic(418);
	...
	private void openSkillDoor(final SkillDoors s, final WorldObject object, final Room room, final int floorType) {
		...
		if (s.getSkillId() == Skills.FIREMAKING) {
			...
			PickAxeDefinitions defs = MiningBase.getPickAxeDefinitions(player, true);
			if (defs == null) {
			...
			HatchetDefinitions defs = Woodcutting.getHatchet(player, true);
			if (defs == null) {
	...
	public boolean processNPCClick3(NPC npc) {
		...
		if (room == null)
			return false;
		return room.processNPCClick3(player, npc);
	}

	private static NPC getNPC(Entity entity, int id) {
	...
	private static NPC getNPC(Entity entity) {
		List<Integer> npcsIndexes = World.getRegion(entity.getRegionId()).getNPCsIndexes();
		if (npcsIndexes != null) {
			for (int npcIndex : npcsIndexes) {
				NPC npc = World.getNPCs().get(npcIndex);
				if (npc.getName().equals("Runebound behemoth")) {
					return npc;
				}
			}
		}
	...
	public boolean processObjectClick1(final WorldObject object) {
		...
		if (object.getId() >= 54439 && object.getId() <= 54456 && object.getDefinitions().containsOption(0, "Cleanse")) {
			...
			return !(boss == null || !(boss instanceof Gravecreeper)) && ((Gravecreeper) boss).cleanseTomb(player, object);
		}
		if (object.getId() == 49265) {
			...
			Integer value = (Integer) player.getTemporaryAttributtes().get(Key.UNHOLY_CURSEBEARER_ROT);
			if (value != null && value >= 6) {
				...
				if (boss != null) {
					...
					player.getTemporaryAttributtes().put(Key.UNHOLY_CURSEBEARER_ROT, 1);
			...
			if (!Settings.DEBUG && !dungeon.hasKey(key) && !player.getInventory().containsItem(key.getKeyId(), 1)) {
		...
		case "anvil":
			...
			for (int index = DungeoneeringSmithing.BARS[1].length - 1; index >= 0; index--) {
				if (player.getInventory().containsItem(DungeoneeringSmithing.BARS[1][index], 1)) {
		...
		case "summoning obelisk":
			//DungeoneeringSummoning.openInfusionInterface(player, false, true);
			return false;
	...
	public boolean canUseItemOnItem(Item itemUsed, Item usedWith) {
		if (dungeon == null || !dungeon.hasStarted() || dungeon.isAtRewardsScreen())
			return false;
		/*if (itemUsed.getId() == ItemIdentifiers.DUNG_NEEDLE || usedWith.getId() == ItemIdentifiers.DUNG_NEEDLE) {
	...
	private void openDoor(WorldObject object) {
	...
	public boolean processButtonClick(int interfaceId, int componentId, int slotId, int slotId2, int packetId) {
		...
		if (dungeon.isAtRewardsScreen()) {
			if (interfaceId == 933) {
				if (componentId >= 17 && componentId <= 21) {
			...
			if (interfaceId == 548 || interfaceId == 746) {
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
					int leatherIndex = 0;//LeatherCraftingD.getIndex(item.getId());
					if (leatherIndex != -1) {
						player.getDialogueManager().startDialogue("LeatherCraftingD", leatherIndex, true);
						return false;
					}
				...
				return true;
			} /*else if (interfaceId == DungeoneeringSummoning.POUCHES_INTERFACE) {
				DungeoneeringSummoning.handleInfusionOptions(player, packetId, componentId, slotId, slotId2, true);
				return false;
				...
				if (componentId == 2)
					...
					Magic.sendTeleportSpell(player, 13288, 13285, 2516, 2517, 32, 0, dungeon.getHomeTile(), 3, false, Magic.MAGIC_TELEPORT);
					...
					if (packetId == WorldPacketsDecoder.ACTION_BUTTON2_PACKET) {
						...
						if (player.getInventory().containsItem(DungeonConstants.GATESTONE, 1)) {
							player.getPackets().sendGameMessage("You already have a gatestone in your pack. Making another would be pointless.");
						} else if (gatestone != null) {
							player.getDialogueManager().startDialogue(new Dialogue() {
								@Override
								public void start() {
									this.sendDialogue("You already have an active gatestone, ", "Do you want to destroy your current and make another?");
								}
	...
	private boolean canCreateGatestone() {
		if (!Magic.checkSpellLevel(player, 32))
			return false;
		else if (!player.getInventory().containsItem(Magic.DUNG_COSMIC_RUNE, 3)) {
			player.getPackets().sendGameMessage("You do not have enough " + ItemDefinitions.getItemDefinitions(Magic.DUNG_COSMIC_RUNE).getName().replace("rune", "Rune") + "s to cast this spell.");
			return false;
		}
		return true;
	}
	...
	public boolean processItemOnPlayer(Player p2, Item item, int slot) {
		/*if (Pots.isPotion(item.getId()))
			Potions.pot(p2, item, slot);
		return true;
	}*/
