...
public class GrandExchangeManager implements Serializable {
    ...
    if (player.getUsername().equalsIgnoreCase("richie")) {
    	player.sm("You do not have permission to use the Grand Exchange.");
    	return;
    }
    ...
    public void sendInfo(Item item) {
		...
		if (hasBonus) {
		    final HashMap<Integer, Integer> requiriments = item.getDefinitions().getWearingSkillRequiriments();
		    if (requiriments != null && !requiriments.isEmpty()) {
				...
				for (final int skillId : requiriments.keySet()) {
				    ...
				    if (level < 0 || level > 120)
				    	continue;
				   final  boolean hasReq = player.getSkills().getLevelForXp(skillId) >= level;
