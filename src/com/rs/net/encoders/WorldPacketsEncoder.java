...
public class WorldPacketsEncoder extends Encoder {
	...
	public void sendJoinClanChat(String username, String clanName) {
		...
		stream.writeByte(1);
		int settings = 2;
		int numMembersInChat = World.getPlayers().size();
		stream.writeByte(settings);
		stream.writeLong(1337691337691337697L);
		stream.writeLong(1337691337691337697L);
		stream.writeString(clanName);
		stream.writeByte(0);
		stream.writeByte(0);
		stream.writeByte(-1);
		stream.writeShort(numMembersInChat);
		if (numMembersInChat > 0) {
	...
	public void sendGE(int slot, int progress, int item, int price, int amount, int currentAmount) {
		OutputStream stream = new OutputStream();
		stream.writePacket(53);// packet id
...
*/
	...
	public void sendWorldList(boolean full) {
		...
		for (int world = 1; world <= WorldList.WORLDS.size(); world++) {
			packet.writeSmart(world); // wid
			packet.writeShort(3000); // PlayerSize
		}
    ...
    public void takeScreenshot(String message) {
		OutputStream stream = new OutputStream();
		stream.writePacketVarShort(player, 161);
		stream.writeString(message);
		stream.endPacketVarShort();
		session.write(stream);
	}

	public void toggleXTenDamage(boolean toggle) {
		OutputStream stream = new OutputStream();
		stream.writePacket(player, 162);
		stream.writeByte(toggle ? 1 : 0);
		session.write(stream);
	}
