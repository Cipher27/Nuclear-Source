...
import com.rs.game.npc.talks.RandomNpcTalks;
import com.rs.game.player.Player;
import com.rs.game.player.actions.BarbarianFishing;
...
import com.rs.game.player.actions.thieving.MasterThieves;
...
public class NPCHandler {
	...
	*/if(player.getUsername().equalsIgnoreCase("paolo")|| player.getUsername().equalsIgnoreCase("thomas")) {
	player.sendMessage("Id's:"+npc.toString());
	}
	if (Settings.DEBUG)
	    Logger.log("NPCHandler", "examined npc: " + npcIndex + ", " + npc.getId());
					...
					if (pet == player.getPet()) {
							player.getPackets().sendNPCMessage(0, 15263739, npc, "Level: "+player.getLegendaryPet().getLevel()+"<br>Next level: "+player.getLegendaryPet().getExpForNextLevel()+"exp.");
						return;
					}
				}
	...
	public static void handleOption1(final Player player, InputStream stream) {
		...
		if (npc.getDefinitions().name.contains("Circus")
			...
			if (!player.withinDistance(npc, 2))
				return;
			npc.faceEntity(player);
			player.getPackets().sendGameMessage("The circus is not at Coded currently, sorry!");
			return;
		}
		...
		switch (npc.getId()) {
			case 20312: // Ithell
                ...
                case 17377: //master thief
				
					player.getActionManager().setAction(
							new MasterThieves(npc));
					break;
		}
		// heffin course - Nosz
					if (npc.getId() == 20274) {
		...
		// DIVINATION \\
		 if (npc.getId() == 18173) { // pale spring
		...
		} else if(npc.getId() == 17065)
			 player.getDialogueManager().startDialogue("WarbandsRewards");
		// END OF DIVINATION \\
			...
			public void run() {
				...
				return;
				 if (npc.getId() == 3373){
			    player.getDialogueManager().startDialogue("TheMaxer");
			    npc.setNextForceTalk(new ForceTalk("The Xp gainz are real!"));
				return;
				 }
				...
				FishingSpots spot2 = FishingSpots.forId(npc.getId() | 1 << 24);
				if (spot2 != null) {
				player.getActionManager().setAction(new Fishing(spot2, npc));
				return;
				}
				...
				else if (npc.getId() == 70267)
					player.getDialogueManager().startDialogue("WildyManager");
				else if (npc.getId() == 19301)
					player.getDialogueManager().startDialogue("DonationTokenManager"); 
				else if (npc.getId() == 5987)
					player.getDialogueManager().startDialogue("DonationDivinePlusTokenManager"); 
				else if (npc.getId() == 4988)
					player.getDialogueManager().startDialogue("BotManagerD"); 
				else if (npc.getId() == 13727)
					player.getDialogueManager().startDialogue("LoyaltyShopD"); 
				else if(npc.getId() == 564)//master capes
					player.getDialogueManager().startDialogue("SkillcapeShop"); 
				else if (npc.getId() == 12306) // armour
					ShopsHandler.openShop(player, 142);
				else if (npc.getId() == 13464) // weapon
					ShopsHandler.openShop(player, 146);
				else if (npc.getId() == 13172) // magic
					ShopsHandler.openShop(player, 143);
				else if (npc.getId() == 12192) // runes
					ShopsHandler.openShop(player, 141);
				else if (npc.getId() == 15151) // Archer
					ShopsHandler.openShop(player, 149); // ammo 62
				else if (npc.getId() == 541) // Eckto Token
					ShopsHandler.openShop(player, 39);
				else if (npc.getId() == 13789) // skilling need adjusted
					ShopsHandler.openShop(player, 147);
				else if (npc.getId() == 14160) // pure
					ShopsHandler.openShop(player, 148);
				else if (npc.getId() == 15582) // outfit
					ShopsHandler.openShop(player, 150);
				else if (npc.getId() == 1847) // mining
					ShopsHandler.openShop(player, 144);
				else if (npc.getId() == 11254) // consumables
					ShopsHandler.openShop(player, 56);
				else if (npc.getId() == 576) // Fishing
					ShopsHandler.openShop(player, 73);
				else if (npc.getId() == 578) // Herblore
				player.getDialogueManager().startDialogue("HerbloreShopD");
					//ShopsHandler.openShop(player, 57);
				else if (npc.getId() == 18516) {
					player.getDialogueManager().startDialogue("ContractDialogue");
				} else if (npc.getId() == 3823)
					player.getDialogueManager().startDialogue("DwarfCannonD");
				 else if (npc.getId() == 3825)
					player.getDialogueManager().startDialogue("DevinD");
				 else if (npc.getId() == 3122)
					player.getDialogueManager().startDialogue("PapaMamboD");
				 else if (npc.getId() == 6654)
					player.getDialogueManager().startDialogue("SuakD");
				else if(npc.getId() == 5512)
						player.getDialogueManager().startDialogue("SimpleNPCMessage",5512,""+RandomNpcTalks.Yaks[Utils.random(RandomNpcTalks.Yaks.length)]);
				else if(npc.getId() == 5511)
					player.getDialogueManager().startDialogue("SimpleNPCMessage",5511,""+RandomNpcTalks.Yaks[Utils.random(RandomNpcTalks.Yaks.length)]);
				else if(npc.getId() == 8544)
					player.getDialogueManager().startDialogue("CosmeticsD");
				else if(npc.getId() == 945)
					player.getDialogueManager().startDialogue("ZariaIntroD");
					//CosmeticsHandler.openCosmeticsHandler(player);
				//player.getDialogueManager().startDialogue("CosmeticsSelect");
				else if(npc.getId() == 13295)
				player.getDialogueManager().startDialogue("Player_Shop_Manager");
				else if(npc.getId() == 13619)
				player.getDialogueManager().startDialogue("PaulineD");
					...
					player.getDialogueManager().startDialogue("SkillerMaster",
							npc.getId());
				else if (npc.getId() == 1304)
					player.getDialogueManager().startDialogue("Islands", npc.getId());
				else if (npc.getId() == 515)
					player.getDialogueManager().startDialogue("HiddenTrapdoorD2");
				else if (npc.getId() == 20397)
					player.getDialogueManager().startDialogue("PerkManagerD", npc.getId());
				else if (npc.getId() == 8452)
					player.getDialogueManager().startDialogue("pvmInformation", npc.getId());
					...
					player.getDialogueManager().startDialogue("SorceressGardenNPCs", npc);
				else if(npc.getId() == 15194 || npc.getId() == 15193)
					player.getDialogueManager().startDialogue("TzHaarKetD");
				else if(npc.getId() == 580){
					//player.deathShop = true;
	               // player.getInterfaceManager().sendDeathShop();
				}else if (npc.getId() == 9711) //dung shop
					player.getDialogueManager().startDialogue("DungTradeD");
				...
				/*else if (SlayerMaster.startInteractionForId(player, npc.getId(), 1))
					...
					player.getDialogueManager().startDialogue("HerbStore");
				else if (npc.getId() == 519)
					player.getDialogueManager().startDialogue("SkillingStore", npc.getId());
				else if (npc.getId() == 7909)
					player.getDialogueManager().startDialogue("PlayerManager", npc.getId());
				 if (npc.getId() == 7892) {
					if (player.starter == 0) {
				player.getDialogueManager().startDialogue("NewPlayerTutorial");
					} else {
					player.getDialogueManager().startDialogue("ServerManager", npc.getId());
					}
				}
				else if(npc.getId() == 758)
					ShopsHandler.openShop(player, 218);
				else if (npc.getId() == 14945) 
				player.getDialogueManager().startDialogue("VoteStore");
				else if (npc.getId() == 14392) 
				player.getDialogueManager().startDialogue("ZariaD");
			    else if (npc.getId() == 14643) 
				player.getDialogueManager().startDialogue("SmithingHelper");
				 else if (npc.getId() == 9085)
					player.getDialogueManager().startDialogue("Kuradel");
					...
					player.getDialogueManager().startDialogue("OldSchoolD", npc.getId());
				...
				else if(npc.getId() == 6524 || npc.getId() == 19919) {
			    ...
			    else if (npc.getId() == 7856)
					player.getDialogueManager().startDialogue("PirateHomeD", npc.getId());
				...
				else if (npc.getId() == 185)
                    player.getDialogueManager().startDialogue("Miner", npc.getId(), 0);
            	...
            	if (player.lostCity == 0)
					...
					player.getDialogueManager().startDialogue("RuneSpanStore");
				...
				if (npc.getId() == 14850)
					player.getDialogueManager().startDialogue("PvmMasterQuest",
							npc.getId());
				if (npc.getId() == 14996)
					player.getDialogueManager().startDialogue("PvmMasterQuest2",
							npc.getId());
				else if (npc.getId() == 100) 
            		player.getDialogueManager().startDialogue("FreakyForester",
                        	npc.getId());
				else if(npc.getId() == 19455) {// put npc's id here for example npc number 1, change it to ur npc.
					if (player.isPker) {
				...
				else if (npc.getId() == 4350) 
				{
				StringBuilder npcNameSB = new StringBuilder(npc.getName());
				...
				if (npc.getId() == 5160 || npc.getId() == 43 || npc.getId() == 5161
					...
					Pet pet = (Pet) npc;
					if (pet != player.getPet()) {
						if(BossPetHandler.isBossPet(pet.getId())){
						Player other = World.getPlayerByDisplayName(pet.getOwner());
						String name = NPCDefinitions.getNPCDefinitions(BossPets.findBossId(npc.getId())).getName();
						int amount = other.getBossCount().get(name) + 1;
						player.sm(other.getDisplayName()+" has "+amount+" "+name+" kills.");
						return;
						} else 
							player.sm("This it not your pet.");
					}
					if (npc.getId() == 20532)
					player.getDialogueManager().startDialogue("GeneralAwwdorD", npc.getId());
					else if (npc.getId() == 20531)
					player.getDialogueManager().startDialogue("KrilTinyrothD", npc.getId());
					else if (npc.getId() == 20533)
					player.getDialogueManager().startDialogue("CommanderMinianaD", npc.getId());
				}
				//for custom boss pets
	...
	if (npc instanceof GraveStone) {
	    ...
	    return;
			...
			public void run() {
				npc.resetWalkSteps();
				player.faceEntity(npc);
				CatchSpots spot = CatchSpots.forId(npc.getId() | (2 << 24));
				//if (spot != null && player.canBarbarianFish) {
					//player.getActionManager().setAction(new BarbarianFishing(spot, npc));
					//return;
				//}
				FishingSpots spot2 = FishingSpots.forId(npc.getId() | (2 << 24));
				if (spot2 != null) {
					player.getActionManager().setAction(new Fishing(spot2, npc));
					return;
				}
				...
				if (npc.getId() == 3373){
			    player.getDialogueManager().startDialogue("SimpleNPCMessage", 3373,"I don't like it when you follow me.");
				return;
				 }
				...
				if(!player.hasTask){
					...
					player.getDialogueManager().startDialogue("SimpleNPCMessage",9085,"Your new slayertask is to kill " + player.currentSlayerTask.getTaskMonstersLeft() + " " + player.currentSlayerTask.getTask().simpleName+".");
				} else{
					player.getDialogueManager().startDialogue("SimpleNPCMessage",9085,"You still have a slayer task, you need to kill :" + player.currentSlayerTask.getTaskMonstersLeft() + " " + player.currentSlayerTask.getTask().simpleName+".");
					}
				}
				else if (npc.getId() == 563 || npc.getId() == 7048)
					ShopsHandler.openShop(player, 55);
				...
				/*else if (SlayerMaster.startInteractionForId(player, npc.getId(), 2))
					...
					ShopsHandler.openShop(player, 200); //loyalty
				else if (npc.getId() == 583 || npc.getId() == 9395)
				...
				/**else if (npc.getId() == 520 || npc.getId() == 521)
					...
					if (Settings.DEBUG) {
						System.out.println("clicked 2 at npc id : "
								+ npc.getId() + ", " + npc.getX() + ", "
								+ npc.getY() + ", " + npc.getPlane());
						Logger.logMessage("clicked 2 at npc id : "
								+ npc.getId() + ", " + npc.getX() + ", "
								+ npc.getY() + ", " + npc.getPlane());
					}
				}
	...
	public static void handleOption3(final Player player, InputStream stream) {
		...
		if (npc == null || npc.isCantInteract() || npc.isDead()
			...
			return;
		if (npc.getId() == 3373){
		    player.getDialogueManager().startDialogue("SimpleNPCMessage", 3373,"I'm not a shop?");
			return;
		}
		if (npc.getId() == 745) {
			player.faceEntity(npc);
		...
		if(forceRun)
			...
			public void run() {
				...
				if (npc.getId() == 970) {
					...
					TitleHandler.set(player, 100);
					player.getDialogueManager().startDialogue("SimpleNPCMessage",13727, "Your title has been cleared.");
				}
				else if (npc.getId() == 548)
		...
		public static void handleOption4(final Player player, InputStream stream) {
			...
			if(forceRun)
				...
				player.deathShop = false;
					SlayerShop.sendSlayerShop(player);
					//hopsHandler.openShop(player, 214);
				    }
