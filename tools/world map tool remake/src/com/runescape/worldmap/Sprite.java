...
/**
 * Class: Sprite.java
 * Originally: Class1_Sub1_Sub1_Sub1.java
 * */
...
public class Sprite extends DrawingArea {
	...
	// Stupid method decompiled with jode.
	public void method36(Font font, FontMetrics fontmetrics, char c, int i,
			boolean bool, RSApplet applet) {
		...
		Image image = applet.getGameComponent().createImage(i_0_, i_3_);
		...
		PixelGrabber pixelgrabber = new PixelGrabber(image, 0, 0, i_0_, i_3_,
				is, 0, i_0_);
		try {
		...
		pixels[i * 9] = (byte) (anInt199 / 16384);
		pixels[i * 9 + 1] = (byte) (anInt199 / 128 & 0x7f);
		pixels[i * 9 + 2] = (byte) (anInt199 & 0x7f);
		pixels[i * 9 + 3] = (byte) (i_7_ - i_5_);
		pixels[i * 9 + 4] = (byte) (i_8_ - i_6_);
		pixels[i * 9 + 5] = (byte) i_5_;
		pixels[i * 9 + 6] = (byte) (i_2_ - i_6_);
		pixels[i * 9 + 7] = (byte) i_1_;
		pixels[i * 9 + 8] = (byte) i_4_;
	...
	public void method37(String s, int i, int j, int k, boolean flag) {
		try {
			...
			for (int l = 0; l < s.length(); l++) {
				...
				if (flag) {
					drawSprite(i1, i + 1, j, 0, pixels, aBoolean198);
					drawSprite(i1, i, j + 1, 0, pixels, aBoolean198);
				}
				drawSprite(i1, i, j, k, pixels, aBoolean198);
				i += pixels[i1 + 7];
			}
	...
	public Sprite(int i, boolean flag, RSApplet applet_sub1) {
		aBoolean198 = false;
		anInt199 = 0;
		pixels = new byte[0x186a0];
		anInt199 = 855;
		aBoolean198 = false;
		...
		for (int j = 0; j < 95; j++)
			method36(
					...
					"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!\"\243$%^&*()-_=+[{]};:'@#~,<.>/?\\| "
							.charAt(j), j, false, applet_sub1);

		if (flag && aBoolean198) {
			anInt199 = 855;
			aBoolean198 = false;
			Font font1 = new Font("Helvetica", 0, i);
			FontMetrics fontmetrics1 = applet_sub1.getFontMetrics(font1);
			for (int k = 0; k < 95; k++)
				method36(
						font1,
						fontmetrics1,
						"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!\"\243$%^&*()-_=+[{]};:'@#~,<.>/?\\| "
								.charAt(k), k, false, applet_sub1);
			...
			if (!aBoolean198) {
				...
				for (int l = 0; l < 95; l++)
					method36(
							...
							fontmetrics1,
							"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!\"\243$%^&*()-_=+[{]};:'@#~,<.>/?\\| "
									.charAt(l), l, true, applet_sub1);

			}
		}
		byte abyte0[] = new byte[anInt199];
		for (int i1 = 0; i1 < anInt199; i1++)
			abyte0[i1] = pixels[i1];

		pixels = abyte0;
	}

	public void drawCenteredString(String s, int i, int j, int k, boolean flag) {
		int l = method41(s) / 2;
		int i1 = method44();
		if (i - l > DrawingArea.extentX)
			return;
		if (i + l < DrawingArea.boundX)
			return;
	...
	public void method39(int ai[], byte abyte0[], int i, int j, int k, int l,
	...
	public int method41(String s) {
		...
		for (int j = 0; j < s.length(); j++)
			if (s.charAt(j) == '@' && j + 4 < s.length()
					&& s.charAt(j + 4) == '@')
				j += 4;
			else if (s.charAt(j) == '~' && j + 4 < s.length()
					&& s.charAt(j + 4) == '~')
				j += 4;
			else
				i += pixels[anIntArray201[s.charAt(j)] + 7];

		return i;
	}

	public void drawSprite(int i, int j, int k, int l, byte pixelsAsAByte[],
			boolean dummy) {
		int i1 = j + pixelsAsAByte[i + 5];
		int j1 = k - pixelsAsAByte[i + 6];
		int k1 = pixelsAsAByte[i + 3];
		int l1 = pixelsAsAByte[i + 4];
		int i2 = pixelsAsAByte[i] * 16384 + pixelsAsAByte[i + 1] * 128
				+ pixelsAsAByte[i + 2];
		int j2 = i1 + j1 * DrawingArea.width;
		int k2 = DrawingArea.width - k1;
		int l2 = 0;
		if (j1 < DrawingArea.boundY) {
			int i3 = DrawingArea.boundY - j1;
			l1 -= i3;
			j1 = DrawingArea.boundY;
			i2 += i3 * k1;
			j2 += i3 * DrawingArea.width;
		}
		if (j1 + l1 >= DrawingArea.extentY)
			l1 -= ((j1 + l1) - DrawingArea.extentY) + 1;
		if (i1 < DrawingArea.boundX) {
			int j3 = DrawingArea.boundX - i1;
			k1 -= j3;
			i1 = DrawingArea.boundX;
			i2 += j3;
			j2 += j3;
			l2 += j3;
			k2 += j3;
		}
		if (i1 + k1 >= DrawingArea.extentX) {
			int k3 = ((i1 + k1) - DrawingArea.extentX) + 1;
			k1 -= k3;
			l2 += k3;
			k2 += k3;
		}
		if (k1 > 0 && l1 > 0)
			if (dummy)
				method39(DrawingArea.pixels, pixelsAsAByte, l, i2, j2, k1, l1,
						k2, l2);
			else
				plotletter(DrawingArea.pixels, pixelsAsAByte, l, i2, j2, k1, l1,
						k2, l2);
	}

	public void plotletter(int ai[], byte abyte0[], int i, int j, int k, int l,
			int i1, int j1, int k1) {
		try {
			int l1 = -(l >> 2);
			l = -(l & 3);
